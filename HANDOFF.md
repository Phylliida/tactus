...
**Honest checkpoint — `contains_key` sits on a thick deep-view stack.** Unlike RC4
(one clean mechanism), the headline RC2 fix peeled back to reveal: (3) the
broadcast lemma `axiom_hashmap_deepview_borrow` projects in its *ensure* clause
(augment_function rewrites params/ret/body, not require/ensure); (4) View-instance
synthesis in the deep_view body (`View (HashMap …) ?V`, fully polymorphic); (5)
`Classical.epsilon` needs `[Nonempty Key]`. (3) is tractable (extend
augment_function to require/ensure); (4)/(5) are deeper and map-specific. Banked
the general mechanism (the real answer to the generality question) and checkpointed
before grinding (4)/(5) — they're a separate, uncertain sub-arc. `HashSet::contains`
(no deep view) fully verifies; the general mechanism also benefits any future
standalone generic spec fn with assoc-type projections.

**Discipline note.** *The probe-then-build loop kept surfacing the next masked
layer.* Each fix (signature lift → implicit binders) revealed the error beneath it
that the prior error had hidden — projection-as-accessor → caller arity → View
synthesis / Nonempty. Reading the regenerated `.lean` after each step (not
reasoning forward) is what exposed the stack honestly rather than declaring victory
at the first green-looking signature.
