...
**482 end-to-end tests + 268 lean_verify unit tests + 66 rust_verify unit tests + 7 integration tests pass — full e2e suite green, 0 failures, 0 crate warnings.** (Structural: `lean_verify`'s unit tests now live in `src/tests/<stem>.rs` via `#[cfg(test)] #[path] mod` — see § "Crate structure". And **`SERVER.md`** holds the feasibility study + plan for a Lean-infoview-style "Tactus server" giving inline goal state on `.rs` source — **Phase-0 de-risk now run and GREEN**, **Phase-1 component 1 (`--emit-lean`) landed**.) Latest (2026-06-02): **loop-invariant hypothesis splitting + consistent `as nat` materialization (BUG-ch5-pow-iter Frictions 1 & 2) + the cast-rendering decision.** Two lowering frictions from the tutorial's exponentiation-by-squaring capstone. **Friction 1:** `walk_loop` glued a loop's invariant clauses into one `∧` hypothesis (`_h_ctx_3 : (P1 ∧ P2 ∧ …)`), so non-splitting user tactics (`nlinarith` / `linarith` / `assumption`) couldn't reach a buried fact; now each clause pushes as its own `CtxFrame::Hyp` frame (`_h_ctx_3..6` individually), for both the maintain (entry) and use (exit) contexts. (Core `omega` / `simp_all` DO decompose conjunction hypotheses — verified empirically — so this mainly unblocks user closers/asserts using non-splitting tactics.) Pinned by `test_exec_loop_invariant_clauses_split_for_user_tactic` (`assert(acc <= i) by { intros; assumption }` where `acc <= i` is one of three invariant clauses — core, no Mathlib, confirmed load-bearing by stash-revert). **Friction 2 (the `as nat` inconsistency):** Verus *elides* the `uN → nat` cast on a bare variable (`result as nat` → `result : Int`, a u64 is trivially a nat) but *keeps* it on a compound (`(result*b) as nat` → `(result*b).toNat`, the inner product is unbounded `Int` in spec mode) and re-materializes it on call args — so a loop invariant `(result as nat) * pow(…)` rendered in `ℤ` while a structurally-identical assert rendered in `ℕ`, and the two wouldn't combine with the `ℕ` spec-fn facts a proof produces. The enclosing arith op keeps its `IntRange Nat` even over the elided operand (`ArithOp Mul (.. IntRange Nat)` over a `U(64)` operand — confirmed via `--log vir-simple`), so generalize the existing `insert_nat_coercions` pass with a `BinaryOp::Arith` arm (SST + VIR-AST): a nat-typed arith op materializes any Int-rendering operand to `Int.toNat`, using the op's own result type (`e.typ`, which for arith ops IS the operand type) as the coercion target via the same `needs_nat_coercion` predicate the call-arg path uses. `(x as nat) * f(…)` now renders `Int.toNat x * f …` uniformly; the pow invariant `_h_ctx_5` flips `ℤ → ℕ` (`result * ↑(pow …)` → `result.toNat * pow b.toNat e.toNat = pow base.toNat exp.toNat`). Faithful (honors the written cast instead of dropping it), visible in output, keeps `nat → Nat` (NOT Option B), not a closer extension; sound by construction (`Int.toNat x` with `0 ≤ x` in scope is exactly what `as nat` denotes) and zero blast radius (fires only on nat-typed arith ops with Int operands, which arise only from `as nat`-typed spec arithmetic). Pinned by `test_exec_arith_operand_as_nat_materializes` (`(x as nat) * (x as nat) == sq(x as nat)` closes via `simp only [sq]` — core, no Mathlib — only when the LHS materializes to match `sq`'s unfolded `ℕ` body; confirmed load-bearing by stash-revert). **Decision recorded** in the new `DECISION-cast-rendering.md` (resolving the deliberately non-committal `DESIGN-cast-hygiene.md`): keep `nat → Nat`; **reject Option B** (`nat → Int` — rejected for `0 ≤ x` bound proliferation on every nat value + the transparency regression of demoting type-intrinsic non-negativity to an external hypothesis; the same faithfulness principle as the wrapper-types-over-peeling and keep-SMT-temps decisions); **reject an auto `tactus_auto` cast-folding rung** (Option A — cuts against minimal-automation, and it fixes *closing* not *rendering*); **fix Friction 2 via consistent materialization instead** (the third path that turned out to be neither B nor a closer hack and to actually work). **Does NOT** fix Friction 3 (`↑e.toNat - 1` vs `e - 1` index noise in unfolded recursive spec fns) or cast-free comparisons (`result * b <= pow(…)`, no `as nat`, no arith-op signal), which stay user-controlled via explicit `as nat`. 480 → 482 e2e, 268 lib, 0 regressions, vstd 1530/0. See § "U → Nat coercion at Call sites" in DESIGN (extended) + `DECISION-cast-rendering.md`. Prior (2026-06-02): **Tactus server — Phase-0 de-risk GREEN + `--emit-lean` codegen-only mode landed.** Two de-risk spikes (`server-spike/`, reproducible) answered SERVER.md's load-bearing unknowns: `lean --server` resolves Mathlib for an *out-of-Lake* `.lean` via `LEAN_PATH` and `$/lean/plainGoal` returns a real goal (so the whole server is viable — *"a real goal → the whole project is green"*); and the tactic-only `didChange` fast path re-elaborates in ~0.5s with warm imports, no rustc (`nlinarith`'s ~11s restore is its genuine proof-search cost, not a fast-path failure). Then built the first real piece: **`--emit-lean`** runs Tactus codegen, writes per-fn `.lean` + a `sourcemap.json` sidecar (the `.rs`↔`.lean` map the infoview consumes), and **SKIPS the Lean run** (no Lean/Mathlib needed; fast). `check_proof_fn`/`check_exec_fn` factored into `pub emit_*` (returning `EmitOutput = {file_path, source_map, warnings}`) + the run tail; new `lean_verify::sourcemap` serde schema (proof = `rs_tactic_byte_range` + `lean_tactic_start_line/count`; exec = `span_marks` with `lean_line`/`rs_loc`/`kind`); verifier accumulates `SidecarFn`s on `Verifier.tactus_sidecar` (merged across buckets like `count_verified`), writes once at `verify_crate_inner`'s tail to `{lean_out_root}/{crate}/sourcemap.json`. **Bonus fix the sidecar exposed:** the proof branch fired for *every* query op of a tactic proof fn (Body + recommends-check) → duplicate sidecar entry AND a redundant per-query-op Lean re-run in normal mode; gated on `Body(Style::Normal)` (mirroring the exec branch) so a tactic proof fn is checked/emitted exactly once. Pinned by `test_emit_lean_codegen_only` (e2e); sidecar inspected, matches SERVER.md's schema. 477 → 480 e2e, 268 lib, 0 regressions, vstd 1530/0. See **SERVER.md** (component 1 LANDED) + `server-spike/`. Prior (2026-06-01): **proof-fn helper files no longer over-include — BUG-proof-fn-dep-walker-over-includes.md FIXED** (then lens-reviewed: `String`→`&str` in `candidates`, + comment-mention-exclusion & diamond coverage tests). When verifying a proof fn, its `.lean` file must contain the helper proof fns it calls (`have h := helper`). The old dep walker emitted *every* proof fn into *every* file; with the root emitted last, any over-included proof fn that *depended on* the root forward-referenced it (`use_double_nonneg` declared before `double_nonneg` in `double_nonneg.lean` → "Unknown identifier"). Correction to the bug doc's suggested fix: `collect_references` *can't* find these edges — a proof fn body is raw Lean tactic text, so the `have := helper` calls exist only as text, not as VIR. Fix: for proof-fn files, `helpers_to_emit` is the root's **transitive downward closure** found by **textually scanning** each tactic body for proof-fn short-names (`ident_appears`, word-boundary, `--`-comment-stripped), in **topological order** (deps first) via DFS post-order — closure + ordering fall out of one pass. `double_nonneg.lean` now holds only `{double, double_nonneg}`; `use_double_nonneg` (its caller) is gone → no forward ref. Does both the doc's (1) over-inclusion fix and (2) proper topo sort. Exec-fn files keep the safe over-approximation (a helper can't forward-reference an exec root). Pinned by `test_proof_fn_helper_not_over_included` + `test_proof_fn_helper_chain_topo_order` (3-level A←B←C). 475 → 477 e2e, 268 lib, 0 regressions, 0 warnings, vstd 1530/0. See § "Proof-fn helper emission" in DESIGN. Prior (2026-06-01): **self-assignment overflow bounds now reach their obligations — BUG-synthetic-temp-let-blocks-asserted-bounds.md FIXED via one `tactus_auto` rung.** For a self-assignment `x = f(x)`, Verus snapshots the operand into a `VirTemp` (`let tmp% := x`) and states the overflow obligation about `tmp% * k`, while the user's asserted bound is about `x * k`; bare `omega` doesn't substitute the `let`-bound value, so the bound couldn't reach the obligation (factorial / power / mod-mul iterative impls all hit this). Fix: lift the `simp_all <;> first | omega | done` combo (which zeta-reduces the `let` then does the arithmetic) out of `tactus_case_split`'s datatype-local gate into a standalone rung — it already existed but was unreachable for `Int`-typed let-guarded goals. Strictly subsumes the bare `simp_all` rung; NOT a simp-set extension (composition only — principle #1 holds); the "layered composition, not exclusive gates" shape; general over any let-guarded linear-arith goal. **A surgical SST→SST pre-pass that *removed* `tmp%` was prototyped and rejected** (Danielle's call: keeping `tmp%` consistently is more transparent than special-casing its removal) — it shipped a soundness bug (18 regressions: branch-result merge temps like `tmp%3 := tmp%1` are shape-identical to snapshots but escape their block), then needed three guards + whole-fn escape analysis to be sound, and was still inconsistent (kept complex-RHS temps) and incomplete (those still block `omega`, so the closer rung was needed anyway). The `tmp%` is faithful to Verus's lowering; absorbing it uniformly in the closer is simpler, sound, general, predictable. Pinned by `test_self_assign_mul_overflow_bound`. 474 → 475 e2e, 268 lib, 0 regressions, vstd 1530/0 (unaffected — Z3 path). See § "Self-assignment snapshot temps" in DESIGN. Prior (2026-06-01): **`HashMap::contains_key` VERIFIES end-to-end — #122 layer 7 (the last deep-view-body blocker) LANDED.** New `lean_verify/src/inline_spec.rs` mirrors Verus's `sst_elaborate` on the VIR-AST: it inlines every `#[verifier::inline]` spec-fn call (the precise + sole divergence axis — Verus inlines `#[inline]` fns in the SST only, so exec goals saw them unfolded while Tactus's VIR-AST renderer left proof-fn goals / bc-lemma clauses folded; `simp_all` can't delta-reduce a `noncomputable def`). One krate-level pass before dep_order + rendering: inline in every fn's require/ensure/returns/decrease/body (recursive, bottom-up), drop the inherent (`Static`) inline fns' defs (no SMT symbol, matching Verus), keep `TraitMethodImpl` inline defs (the trait instance needs them). Re-stamps the inlined result with the call site's type so ref decorations survive (mirroring `sst_elaborate`'s `SpannedTyped::new(&exp.span,&exp.typ,…)`), peels the trivial `{expr}` block, substitutes both by-value `Var` params and by-ref `ReadPlace(Local(self))` receivers. Wired by shadowing `krate` (+ re-fetching the inlined root) at the top of `check_proof_fn`/`check_exec_fn`. Closes under DEFAULT `tactus_auto` (no simp-set extension — Principle #1 held). One consequence handled: consistent (`index`-form) `Seq` broadcast axioms *activate* the previously-dormant seq update/ext rewrites, derailing `simp_all` on the `vec_index` prophecy goal (a close that passed by relying on the non-faithful dormancy) — that test gets a `#[verifier::tactus_tactic("intros; grind")]` closer (transparent, visible, no vstd-internal names). The former graceful-degradation pin is renamed `test_cross_crate_map_contains_key` (Ok); `inline_spec::tests::inline_okay_matches_verus` unit-pins the drop/inline gate. **Then review-hardened + comprehensively covered** (same session): a review pass found no live bugs but turned four *unchecked invariants* Verus enforces into fail-loud guards (`assert_eq!` on the substitution lengths, exhaustive `inline_okay`, broadened `body_blocks_inlining`, common-case early-out), and a coverage cluster *constructed* cases for the documented latent gaps — all handled correctly (`test_inline_fn_field_access` proved field access inlines without dangling, **disproving an over-cautious comment I'd just written**; `_nested_chain`, `_trait_impl_method` [the keep-def branch], `_binder_body_kept_folded`). 474 e2e + 268 lib, 0 regressions, vstd 1530/0. See § "Cross-crate `HashMap::contains_key` deep-view-body arc" layer 7 in DESIGN. Prior (2026-05-31): **`HashMap::contains_key` deep-view-body arc — layers 4–6 landed; the deep-view body now fully elaborates, only layer 7 (fold/unfold) remains.** `hash_map_deep_view_impl` is the first cross-crate spec fn whose BODY Tactus fully renders + elaborates; probing it (generate the `.lean`, read live errors, drive to zero) found three elaboration fixes plus one obligation-level deferral. Decision (Danielle's gate): **keep the body, don't axiomatize** (axiomatizing the opaque def is sound but incomplete — loses Verus `reveal`-parity). **(6)** `structural_typ` returns `strip_all_ref_decorations(expr.typ)` for `ExprX::Call`, so a ref-decorated call receiver (`(self@[k]).deep_view()` borrows a `spec_index` result) gets its `.mk` via the existing bridge — VIR-AST analog of the SST renderer's U2 depth-awareness; no-op for undecorated calls (`b23e87f`). **(4)** `dep_order::order_emission` — Kahn's topological sort of spec-fn groups + trait instances (replacing the fixed "instances after all spec fns" phase), so the `View (HashMap …)` instance emits BEFORE `hash_map_deep_view_impl` (which dispatches to it via `m@`) while `DeepView (HashMap …)` stays after (its body references the def). Dispatch edge read from the trait-method call's `CallTarget::Fun` ImplPaths (not `DynamicResolved.impl_paths`, empty here); lowest-id tiebreak → zero drift for instances nothing dispatches to. Pure `kahn_emit` unit-tested (`083acae` + `8282da7`). **(5)** `nonempty.rs` — Verus `choose` is total (sound only because all spec types are inhabited) and renders as `Classical.epsilon`, which Lean requires `[Nonempty T]` for; infers it via dataflow (SEED at `ExprX::Choose` over a type-param, PROPAGATE backward along the call graph mapping type-args, fixpoint), synthesised as `GenericBoundX::Trait(Nonempty,[T])` at augment time (rides `trait_bounds_to_ast`, no class/dep leak), wired to spec fns + bc lemmas via augment + instances via `instance_nonempty_bounds`. Emits the exact binder Lean would force a hand-written version to add, in the exact places, bottoming out at the user's concrete `Key = u8`. Pinned `test_nonempty_choose_over_type_param` (`06ff961`). Probe: 5 errors → 1. **(7) then LANDED 2026-06-01** (`inline_spec`, the new Latest above): the obligation needed `map_lib.contains_key` unfolded to match the bc-lemma; the precise cause was the `#[verifier::inline]` attribute (Verus inlines such fns in the SST only), and mirroring `sst_elaborate` on the VIR-AST closes it — `contains_key` verifies. 470 e2e (+1), 267 lib (+4), 0 regressions, vstd 1530/0. See § "Cross-crate `HashMap::contains_key` deep-view-body arc" in DESIGN. Prior (2026-05-31): **generalized projection-lifting (#122 RC2 — projection mechanism complete).** Tactus emits trait assoc types as `outParam` class params, so a `<X as Trait>::N` projection over an abstract type-param can't render as an accessor — it must be lifted to a fresh binder constrained via the trait's instance bracket (the Bug-B mechanism). That lift was wired only to impl-methods + instance-signatures; now it's a uniform property of **every** generic function (the RC4-analog: generalize the mechanism, don't special-case). New `impl_subst::maybe_augment_standalone_fn` drives the already-generic `ImplSubst::build` from a standalone fn's own typ_params/bounds; `generate.rs`'s spec-fn loop routes non-`TraitMethodImpl` fns through it. Two pieces beyond the prior signature-only scope: (a) **body typ-walk** — `augment_function` now rewrites projections embedded in the body (lambda binder annotations, `Map.new` type-args) via a new pub VIR helper `map_expr_typ_visitor` (the "public TypX walker" DESIGN flagged; #94-style visitor-visibility promotion); (b) **implicit synthetic binders** — the fresh `_tactus_assoc_*` binders render implicit (centralized `ASSOC_BINDER_PREFIX`/`is_assoc_binder`) so VIR call sites passing only the original typ_args still typecheck (Lean infers them via the outParam bracket). Beyond signature+body, the mechanism now also covers (c) **clauses + bounds + broadcast lemmas**: `augment_function` rewrites require/ensure/returns and (via `rewrite_bound`) projections embedded as trait-bound type-args; `generate.rs` applies it to broadcast lemmas too; and `ImplSubst::build`'s Source-2 "uncovered slots" pass now actually checks coverage (skips a slot already pinned by an existing `TypEquality`, instead of synthesising a second binder → over-arity bracket). Strict refinement — no-projection fns get an empty subst → identity; full suite green. **The whole projection cascade is cleared** — the `view.DeepView.V Key`-as-accessor errors on `hash_map_deep_view_impl` (signature/body/caller-arity) *and* the `axiom_hashmap_deepview_borrow` broadcast-lemma errors. After this projection mechanism, `HashMap::contains_key`'s residual was two non-projection deep-view-body issues — which the deep-view-body arc (the current Latest, above) then resolved into the layer-4 ordering fix and the layer-5 Nonempty-binder fix (plus the layer-6 coercion they unmasked), leaving only the layer-7 fold/unfold obligation. `HashSet::contains` fully verifies (RC4). See § "Generalized projection-lifting" in DESIGN. Prior (2026-05-31): **instantiated-param-type call-arg coercion (#122 RC4) — cross-crate `HashSet::contains` verifies; `HashMap::contains_key` now blocked on RC2 alone.** vstd's `axiom_set_contains_box` / `axiom_contains_box` ensure `…borrowed_key(m,k) <==> m.contains(Box::new(*k))`, and Verus erases `Box::new(*k)` to a bare `*k : Q` at the value level (confirmed via `--log vir-simple`: the surviving VIR arg is `ReadPlace(Local k) : TypParam Q`, no box node) — so the key rendered as `k.deref : Q` against the `K ↦ Box<Q>` key slot ("Application type mismatch"). Fix completes the existing call-arg bridge rather than special-casing: `RenderCtx::fn_param_typs` now takes the call's `typ_args` and returns the callee's param types **as instantiated** (`{typ_params[i] ↦ typ_args[i]}` via `vir::sst_util::subst_typ`), so `coerce_lexpr` bridges `Q → Box Q` with a `.mk` wrap — faithfully recovering the erased construction: `set.Set.contains (Box Q) m (Tactus.Box.mk k.deref)`. Applied at all four bridge sites (`call_to_node`, the class-method arm, both SST `CallFun::Fun` arms); `proof_fn_signature` now renders req/ensure with the fn_map in scope (`with_fn_map` not `empty`) so broadcast-lemma axioms get the bridge. Strict refinement — substitutes only on `typ_args.len() == typ_params.len()` (else falls back to declared types → zero change for concrete-param calls), and `coerce_lexpr` is wrapper-only so it can't mask inner mismatches. Transparency evaluated before building (Danielle's gate): faithful reconstruction, type-determined (the call's explicit typ_arg says `K=Box<Q>`), visible in the goal, no trust/closer extension. General over any wrapper-instantiated generic call, not Box-specific. `test_cross_crate_set_contains` (Ok) + `_unemittable_trait_degrades_not_panics` (still Err — RC4 error gone, only RC2/DeepView remains). 468 → 469 e2e, 263 lib, 0 regressions, vstd 1530/0. See § "Instantiated-param-type call-arg coercion" in DESIGN. Prior (2026-05-31): **cross-crate marker-shell trait emission (#122 RC1/RC3) — `HashMap::len` + spec/proof `<T: Clone>` verify.** Un-emittable cross-crate traits (methods stripped, e.g. `core::clone::Clone`) now emit as contentless *marker shells* instead of being skipped (skipping left dangling `[clone.Clone Self]` superclass binders on emittable subclasses → a 22-error cascade); every bound *referencing* a shell trait is dropped at the shared `trait_bounds_to_ast_with` chokepoint (a contentless bound loses no provable fact — same justification as the B3 binder-drop); shell-trait *instances* are skipped (dead once their bounds are dropped, and they dangle on un-reached traits like `marker.Copy`). **RC3**: `collect_referenced_datatypes` also seeds from emitted instance heads so an opaque type reachable only there (`DefaultHasher`) gets its `axiom T : Type`. A review pass centralized the filter at the chokepoint (covering spec/proof-fn generic bounds, not just the class/instance pre-filter sites); a coverage probe then found+fixed a real bug — RC1 had over-emitted shell instances (`{A}[marker.Copy A] : clone.Clone A` dangled on un-reached `marker.Copy`); the bound-drop was the actual fix, the instances were over-build. Map/Set `contains`/`contains_key` still degrade *gracefully* — residual is **RC2** (DeepView outParam projection) + **RC4** (the Box-key wrapper-faithfulness coercion: Verus collapses `Box::new(*k)` to bare `Q` against a `Box<Q>` key param). 465 → 468 e2e, 263 lean_verify lib, 0 regressions, vstd 1530/0. See § "Cross-crate marker-shell trait emission" in DESIGN + the Phase-3 Map/Set bullet. Prior (2026-05-30): **returned-mut-ref prophecy composition — `&mut v[i]` verifies.** `bump(&mut v[0])` lands end-to-end: a prophecy var `P` is minted at the `MutRef`-returning `vec_index_mut` call (rendered as `*final(element)` in its ensures) and reused as the resolving `bump` call's post-state, so the `+1` threads into `v`'s post-state (`view(v)@[0] == update(old@,0,P)[0] == P == old[0]+1`). General over any `MutRef`-typed dest (no `vec_index_mut` special-case); no `tactus_auto` simp-set extension (the closer closed the real goal as-is) — both transparency lines held. `test_exec_call_mut_arg_vec_index` (Ok) + `_wrong_ensures` (Err soundness pin). Reviewed clean: a multi-lens soundness pass (mechanism sound by ∀-binding) added an invalidate-on-resolution guard + restricted reuse to whole-ref targets + 4 adversarial pins; a right-way audit then derived-not-stored the reuse flag and named the `ReturnProphecy` struct (461 → 465 e2e). See § "Returned-mut-ref prophecy composition" in DESIGN. Prior (2026-05-30): **#122 cross-crate View / exec-callee emission.** Four landings (B1–B4 scoping note below, RESOLVED): **B2** trait-method call heads use the full module-qualified path (`view.View.view`, matching the emitted class) instead of the last two segments; **B1** `rewrite_self_sibling_calls` gates on resolved kind (so a blanket impl's cross-instance forward stays class dispatch) plus a forwarding-blanket body synth that faithfully reproduces `(**self).view()` as `view.View.view (Tactus.Ref.mk self.deref.deref)` for Ref/Box/Rc/Arc (recovering the smart-pointer deref Verus leaves opaque); **B3** instance binders the head doesn't determine (the erased `Box<T,A>` allocator param) are dropped so Lean can synthesize; and `has_resolved(x)` renders as an uninterpreted `Tactus.hasResolved` Prop. **This unblocked the whole `v@` (View) dispatch path: `Vec::len`, `Vec::push` (with `&mut` + `old(v)`), and `v[i]` reads all verify soundly under `use vstd::prelude::*`** (probed; wrong-ensures correctly fails). Plus a follow-up: **un-emittable cross-crate trait classes are skipped, not panicked on** — `HashMap` drags in `core::clone::Clone` (methods stripped cross-crate), which used to panic `trait_to_ast`; now such traits + their instances are skipped, so Map/Set code fails gracefully (`tactus_auto failed`) instead of crashing the verifier. A scoping spike on `&mut v[i]` then landed the **BuiltinSpecFun broadcast filter** (FnMut-axiom-pulling code degrades gracefully too) and corrected the frontier: the remaining `&mut v[i]` work is **returned-mut-ref prophecy composition** (a fresh new-mut-ref arc), not the bounded B1-family fix first guessed. A high-effort code review of the whole arc found **no correctness bugs** (all candidates verified REFUTED) and landed a 6-item zero-behavior-change cleanup/hardening pass. (Prior, 2026-05-29: **#122 cross-crate broadcast lemmas** — vstd's `Seq`/`Set`/`Map` semantic lemmas reach the closer from default-on-import + explicit `broadcast use`; default-on-import panic on un-emittable cross-crate trait bounds fixed by skipping such lemmas. Cluster B closed (`ref_to_bare`, `aliased_arg`, `cross_instantiation`); Cluster A closed via typed substitution + universal call-arg bridging.) vstd still verifies (1530 functions, 0 errors). The pipeline works: user writes a proof fn with `by { }` or an exec fn with `#[verifier::tactus_auto]`, Tactus generates typed Lean AST, pretty-prints to a real `.lean` file, invokes Lean (with Mathlib if available), and reports results through Verus's diagnostic system.
...
#### Current session (2026-06-02 — BUG-ch5-pow-iter Frictions 1 & 2 + the cast-rendering decision)

The morning ritual (DESIGN end-to-end — now 5183 lines — + a HANDOFF skim + a
poem, `poems/2026-06-02.md` "setting the lamp down"), then a request to look into
`BUG-ch5-pow-iter-lowering-frictions.md` ("rn multiple result in & of them but
really we should just have them as multiple"). Three frictions block the
tutorial's exp-by-squaring capstone; landed the two that were the right calls,
and recorded a decision on the third.

**Friction 1 — loop-invariant clauses split into individual hypotheses.**
`walk_loop` (`sst_to_lean.rs`) built the maintain/use context with one
`CtxFrame::Hyp(and_all([P1, P2, …]))`, which `split_leading_binders` named as a
single `_h_ctx_3 : (P1 ∧ P2 ∧ …)`. Non-splitting user tactics (`nlinarith`,
`linarith`, `assumption`, `exact`) can't reach a fact buried in a conjunction, so
invariant facts that were "right there" were unreachable in asserts and override
closers. Fix: push one `CtxFrame::Hyp` per clause (entry list → maintain, exit
list → use), so each becomes its own `_h_ctx_N`. SpanMark wrapping unchanged
(per-conjunct marks already existed inside the old `and_all`). Empirically
confirmed (via a standalone Lean probe) that core `omega` / `simp_all` DO
decompose conjunction hypotheses — so the bug's "neither omega nor nlinarith
looks inside a conjunction" is only half-true; the split's clear beneficiaries
are non-splitting *user* tactics. Pinned by
`test_exec_loop_invariant_clauses_split_for_user_tactic` (an `assert(acc <= i) by
{ intros; assumption }` where `acc <= i` is one of three invariant clauses —
`assumption` is core/no-Mathlib and does NOT decompose `∧`, so it discriminates;
confirmed load-bearing by `git stash` of the fix → test flips to Err). Commit
`82c95a6`.

**Mathlib broke mid-session — diagnosed + restored.** The nlinarith/ring/bit_vector
tests were failing with `unknown module prefix 'Mathlib'`. Forensics: every *other*
lake dep (aesop, batteries, Qq, …) still had its `.lake/build`, but
`lean-project/.lake/packages/mathlib/.lake/` held only `config/` — the ~2GB of
precompiled oleans were gone, and the mathlib source had been re-fetched (dir
mtime that day). Toolchains matched (both v4.25.0), so not a version drift — just a
re-fetch of source without `lake exe cache get`. Ran `lake exe cache get` (7525
oleans restored); the 29 Mathlib-flavored failures went green. Worth knowing: a
`lake` re-resolve of mathlib re-clones source but does NOT re-download oleans.

**Friction 2 — consistent `as nat` materialization on arith operands.** Probed the
exact inconsistency: Verus *elides* the `uN → nat` cast on a bare variable
(`result as nat` → `result : U(64)`) but *keeps* it on a compound (`(result*b) as
nat` → `Clip(Nat, result*b)`, the inner product being unbounded `Int` in spec
mode), and re-materializes on call args. So the pow invariant `(result as nat) *
pow(…)` rendered in `ℤ` (`_h_ctx_5 : result * ↑(pow …)`) while a structurally
identical assert rendered in `ℕ` — they wouldn't combine. `--log vir-simple`
confirmed the enclosing op keeps `ArithOp Mul (.. IntRange Nat)` even over the
elided `U(64)` operand, so the re-materialization signal is present. Fix:
generalize `insert_nat_coercions`' leaf (`rewrite_one_call_for_coercions{,_expr}`)
with a `BinaryOp::Arith` arm — a nat-typed arith op materializes any
Int-rendering operand to `Int.toNat`, using the op's own result type (`e.typ`,
which for arith ops IS the operand type) as the coercion target via the existing
`needs_nat_coercion`. The pow invariant flips `ℤ → ℕ`
(`result.toNat * pow b.toNat e.toNat = pow base.toNat exp.toNat`), matching the
asserts; in the probe the else-branch assert's post-`rw` goal became *exactly*
`_h_ctx_5` (couldn't, pre-fix). Faithful (honors the written `as nat`), visible,
keeps `nat → Nat`, not a closer hack; sound (`Int.toNat x` with `0 ≤ x` in scope =
what `as nat` denotes) and zero blast radius (fires only on nat-typed arith ops
with Int operands, from `as nat`-typed spec arithmetic). Pinned by
`test_exec_arith_operand_as_nat_materializes` (`(x as nat)*(x as nat) ==
sq(x as nat)` closes via `simp only [sq]` — core, no Mathlib — only when the LHS
materializes to match `sq`'s unfolded `ℕ` body; confirmed load-bearing by
stash-revert). Commit `06ae4ce`.

**The decision (Danielle's steer, twice corrected my over-reach).** I first pushed
**Option B** (render `nat → Int`) as the clean fix; Danielle: "I thought we decided
not to do this?" — and she was right (the recorded exploration was non-committal,
but the reason holds): `nat → Int` forces a `0 ≤ x` bound on *every* nat value and
demotes type-intrinsic non-negativity to an external hypothesis — a transparency
regression, the same faithfulness principle as wrapper-types-over-peeling and
keep-SMT-temps. Then I over-corrected into "handle at proof site / it's just
cosmetic"; Danielle re-aimed at *consistency* ("casting happens differently in
different places — ideally predictable everywhere"), which turned out to be the
*right* and *tractable* frame and is what the Friction-2 fix delivers. Recorded in
the new **`DECISION-cast-rendering.md`** (resolving `DESIGN-cast-hygiene.md`): keep
`nat → Nat`; reject Option B (`0 ≤ x` + transparency); reject an auto closer rung
(minimal-automation + fixes closing not rendering); fix Friction 2 via consistent
materialization. **Friction 3** (`↑e.toNat - 1` vs `e - 1` index noise) and
cast-free comparisons (`result*b <= pow(…)`) are NOT addressed — user-controlled
via explicit `as nat`. The pattern worth keeping: a question from outside ("didn't
we decide against this?", "isn't it that casting is inconsistent?") made the work
smaller and truer than the grander fix I reached for — twice.

480 → 482 e2e, 268 lib, 0 regressions, vstd 1530/0. See DESIGN § "U → Nat coercion
at Call sites" (extended) + `DECISION-cast-rendering.md`.

#### Earlier session (2026-06-02 — Tactus server: Phase-0 de-risk GREEN + `--emit-lean` landed)

The morning ritual (DESIGN end-to-end + HANDOFF + **SERVER.md** in full), a poem
(`poems/2026-06-02.md`, "the lamp you carry" — arriving at the server doorway),
then the server work itself in two stages: **de-risk, then build.**

**Phase-0 de-risk — both spikes GREEN** (`server-spike/`, reproducible, self-resolving
`LEAN_PATH`). SERVER.md flagged two load-bearing unknowns; both came back favorable:
1. *Does `lean --server` resolve Mathlib for an out-of-Lake `.lean` with `LEAN_PATH` set,
   and return a goal via `$/lean/plainGoal`?* — **yes.** `plaingoal_probe.py` drove the LSP
   server against a real generated file (imports `Mathlib.Tactic.Linarith`, uses `linarith`
   + `nlinarith`), rootUri = the file's own dir (no lakefile in any ancestor → Mathlib
   resolved *purely* via `LEAN_PATH`): processed in ~16s, 0 errors, plainGoal returned the
   full proof state. Cursor "state here" semantics confirmed (cursor at a tactic's start →
   goal before it; past it → `no goals`). Per SERVER.md: a real goal → the whole project is
   green.
2. *Live-edit fast path* — `livedit_probe.py` (talks ONLY to `lean --server`, no rustc):
   cheap tactic edits `didChange` and settle in ~0.5–0.6s with correct live diagnostics,
   imports stay warm (no 16s reload). The ~11.6s restore is `nlinarith`'s genuine
   proof-search cost (same as batch), not a fast-path failure — "Lean-speed" = the speed
   Lean elaborates *that* tactic, like the VS Code Lean4 infoview today.

**Phase-1 component 1 — `--emit-lean` codegen-only mode + `sourcemap.json` sidecar.**
The first real build piece (the foundation the infoview consumes). `--emit-lean` runs
Tactus codegen, writes per-fn `.lean` + a sidecar mapping each `.rs` tactic region to its
`.lean`, and **skips the Lean run** (no Lean/Mathlib; fast). Three steps, each committed
+ verified:
- **emit/run split** (`d365398`, behavior-preserving): `check_proof_fn`/`check_exec_fn` →
  `pub emit_proof_fn`/`emit_exec_fn` returning `Result<EmitOutput, CheckResult>`
  (`EmitOutput = {file_path, source_map, warnings}`); `check_* = emit_* + run tail`.
- **flag** `--emit-lean` (`config.rs` 5-site bool + harness allowlist).
- **wiring** (`4dc6f81`): `lean_verify::sourcemap` serde schema (proof: `rs_tactic_byte_range`
  + `lean_tactic_start_line/count`; exec: `span_marks` {`lean_line`,`rs_loc`,`kind`}), built
  from `EmitOutput` + the `.rs` `tactic_span` in scope at the call site. Verifier's two Tactus
  branches take the emit path under `--emit-lean`, push a `SidecarFn`, accumulate on
  `Verifier.tactus_sidecar` (merged across buckets), write once at `verify_crate_inner`'s tail.
- **Bonus fix the sidecar exposed:** the proof branch fired for *every* query op of a tactic
  proof fn (Body + recommends) → duplicate entry AND a redundant per-query-op Lean re-run in
  normal mode. Gated on `Body(Style::Normal)` (mirroring the exec branch) → checked/emitted
...
Two non-bug pieces of the same long session.

**Test extraction sweep — all inline tests moved to `src/tests/`.** Every
`lean_verify` functionality file's inline `#[cfg(test)] mod tests { … }` body
moved to a `src/tests/<stem>.rs` sibling, leaving a 3-line declaration:
`#[cfg(test)] #[path = "tests/<stem>.rs"] mod <name>;`. Kept as `#[path]`'d `mod`
**children** so `use super::*` still reaches private items (the tests exercise
`Wp`, `build_wp`, etc.) — a crate-wide test file would lose that. Mod names
preserved (`lean_ast` keeps `substitute_tests`). Bodies de-indented one level,
**raw-string-aware** so `sanity.rs`'s multi-line `r#"…"#` literals stay
byte-identical; fixed two `include_str!("../../vir/src/ast_to_sst.rs")` shape-drift
guards to `../../../` (the file moved a dir deeper). Functionality files shrank a
lot — `sst_to_lean.rs` 8774→6624, `lean_ast.rs` 3034→1837, `typed_expr.rs`
644→192. Pure relocation, suite-verified. Plus a `lean_verify` warning sweep (lib
+ `--tests` + doctests now **0 warnings**) and a pre-existing `impl_subst.rs`
doctest fix (```rust → ```text). New DESIGN note in § "Crate structure".

**SERVER.md — Tactus-server feasibility study + implementation plan.** Asked what
it'd take to build a Lean-infoview-style server (inline goal state at cursor) on
`.rs` source. Investigated against live code; **verdict: doable.** Core insight:
it's a *proxy* over `lean --server`, not a reimplementation — Tactus already
emits per-fn `.lean`, and proof-fn tactic bodies are copied **verbatim**, so
cursor↔lean is a constant offset (`tactic_start_line`); tactic edits never touch
VIR (live updates without rustc). `lean --server` + `$/lean/plainGoal` confirmed
present. Full evidence table (file:line), proxy architecture (sidecar `sourcemap.json`
+ extension orchestrates `tactus emit` + `lean --server`), precise components, a
3-phase plan, and a 1–2 day de-risk spike (does `lean --server` resolve Mathlib
for an out-of-Lake file). See **`SERVER.md`** — self-contained for a cold start.

#### Current session (2026-06-01 — proof-fn dep walker: downward closure + topological sort)

**477 e2e (+2), 268 lib, 0 regressions, 0 warnings.** Asked to look into
BUG-proof-fn-dep-walker-over-includes.md, then "yes let's do proper topological
sort."

**Diagnosis.** Confirmed both halves (over-inclusion + forward-ref order).
`generate.rs::krate_preamble` built `helpers_to_emit` as *every* proof fn,
emitted into *every* file; root emitted last → any over-included proof fn that
depended on the root forward-referenced it. The prior code even flagged the
ordering half ("source order works in the common case … topological sort …
deferred until a case surfaces" — this was that case).

**Correction to the bug doc.** Its suggested fix (1) "apply `collect_references`
to proof fns" *wouldn't work*: a proof fn body is raw Lean tactic text
(`TacticBlock` span), so VIR dep-walking can't see the `have := helper` calls —
they exist only as text. That's *why* the original over-included everything (no
VIR signal). Flagged this to Danielle as part of the diagnosis.

**Fix.** For proof-fn files, `helpers_to_emit` = the root's transitive *downward*
closure, found by textually scanning each tactic body for proof-fn short-names
(`ident_appears` — byte-level word-boundary, Unicode-safe; `--`-comments
stripped), returned in topological order (deps first) via DFS post-order
(`collect_referenced_proof_fns`). Closure + ordering fall out of one pass; the
root, being the top of its own DAG, lands last. Does both the doc's (1)
over-inclusion fix and (2) the proper topo sort. Exec-fn files keep the safe
over-approximation — a helper can't forward-reference an exec root (no proof fn
depends on an exec fn), and the exec root's helper refs aren't available at
preamble time anyway. Pinned by `test_proof_fn_helper_not_over_included` (the
reproducer) + `test_proof_fn_helper_chain_topo_order` (3-level A←B←C).

**Design note.** The textual scan is the *only* available dependency signal
(proof bodies are opaque text); it over-approximates safely (a name in a string
false-matches — rare; never under-includes a real call). A true mutual-recursion
cycle would need a Lean `mutual` block — out of scope, not observed. New DESIGN §
"Proof-fn helper emission".

#### Earlier session (2026-06-01 — self-assign overflow bounds: `tactus_auto` rung, NOT a temp-removal pass)

**475 e2e (+1), 268 lib, 0 regressions, vstd 1530/0.** Asked to look into
BUG-synthetic-temp-let-blocks-asserted-bounds.md ("might be addressed now").

**Diagnosis (probe-driven).** Not fixed. Reproduced, drove the generated `.lean`
through Lean directly: for `x = x*(i+1)`, Verus snapshots the operand into a
`VirTemp` (`let tmp% := x`) and states the mul-overflow obligation about
`tmp% * (i+1)`, while the user's `assert(x*(i+1) <= B)` is about `x`. Bare
`omega` doesn't substitute the `let`-value (distinct opaque atoms — even
`intro`-ing the `let` then `omega` fails); bare `simp_all` zeta-reduces but isn't
arithmetic. `simp_all <;> omega` closes it (verified on the extracted goal). That
combo *already existed* in the ladder, trapped behind `tactus_case_split`'s
datatype-local gate. (The bug doc's own minimal repro was doubly inconsistent —
its `x ≤ 1000` invariant isn't inductive under self-multiply — a red herring
separate from the temp bug; isolated a clean consistent reproducer.)

**The fork, and Danielle's steer.** First built the suggested option B — an
SST→SST pre-pass *removing* `tmp%` (substitute `tmp% → x`, drop the decl,
recover human-written `let x := x*(i+1)`). It shipped a **soundness bug: 18
regressions** — a branch-result merge temp `tmp%3 := tmp%1` (assigned in both
`if` arms, read after) is shape-identical to a snapshot but escapes its block;
inlining dropped its assignment on one path → unbound. Making it sound needed
three guards (read-before-write, dead-after-reassign, escape-confinement) + a
whole-fn occurrence count. Danielle: *"do you think these changes are a good
idea? maybe having tmp consistently is more transparent than this special
casing."* She was right, and I said so: the pre-pass is soundness-delicate
(escape analysis to keep "cleaner output" sound is fighting the grain),
inconsistent (removes bare-Var snapshots, keeps complex-RHS ones — unpredictable
"why did this vanish?"), and incomplete (complex-RHS temps still block `omega`,
so the closer rung is needed anyway). **Reverted the pre-pass entirely.**

**Landed (the general fix).** One rung in `TactusPrelude.lean`: lift
`(simp_all <;> first | omega | done)` out of `tactus_case_split` to standalone.
Strictly subsumes the bare `simp_all` rung; composition, not a simp-set
extension (principle #1 holds); the "layered composition, not exclusive gates"
shape; general over any let-guarded linear-arith goal. The `tmp%` stays —
faithful to Verus's lowering, absorbed uniformly in the closer. Pinned by
`test_self_assign_mul_overflow_bound`. New DESIGN § "Self-assignment snapshot
temps" records the keep-don't-remove decision as the dual of "What doesn't have
to mirror Verus's encoding" (remove an SMT encoding when it makes a *class*
uniformly simpler; keep it when removal special-cases instances while leaving the
class half-handled).

**Discipline note.** *Transparency is faithfulness + predictability, not
...
**470 e2e + 268 lib (+1), 0 regressions, vstd 1530/0.** Asked to take Layer 7
(the last deep-view-body blocker) "properly, in the general way, no deferred
stuff."

**The probe-driven diagnosis.** Layer 7's probe-time framing ("two-parallel-
renderers divergence — converge by folding or unfolding") was *near* the truth
but mis-scoped. The precise root: Verus's `sst_elaborate` inlines a spec-fn call
iff `attrs.inline && kind.inline_okay()` — the `#[verifier::inline]` attribute —
and runs on the **SST only**. So exec goals (from the SST) see `#[inline]` fns
unfolded; the VIR-AST renderer (proof-fn goals, bc-lemma clauses) leaves them
folded. Reading the actual `sst_elaborate.rs` (not reasoning from the symptom)
showed **`#[inline]` is the *entire* divergence class** — default/open/closed
stay folded on both paths, opaque is `@[irreducible]`, uninterp primitives have
no body. So mirroring that one Verus pass on the VIR-AST closes the class, not a
`contains_key` patch (the altitude Danielle pushed for: "is there a more general
fix?" → "this *is* the general fix; here's why it's complete").

**Build (`inline_spec`, commit `6ef9b97`).** One krate-level VIR-AST→VIR-AST
pass; details in the current-state Latest above. The probe-then-build loop
surfaced each next layer: pass didn't fire (`#[inline]` stripped? no —
`has_binder` over-flagged the body's trivial `Block`) → unresolved `self` (it's
a `ReadPlace(Local)`, not a `Var`) → lost `Ref.mk` coercion (used the body's
bare type, not the call site's — fixed by re-stamping, exactly as
`sst_elaborate` does) → "fields missing: view" (dropped a `TraitMethodImpl`
inline method the instance needed — so drop only `Static`). Each found by
reading the regenerated `.lean`, never by reasoning forward.

**The one regression, and the call.** The general fix is correct/faithful, but
making the `Seq` broadcast axioms consistent (`index` form, since
`Seq::spec_index` is `#[inline]`) *activates* the seq update/extensionality
rewrites that were **dormant** at baseline via the form mismatch — and their
collective firing derails `simp_all` on the `vec_index` prophecy goal. Bisected
to the exact axioms; the close had been passing by *relying on the non-faithful
dormancy*. Surfaced the fork to Danielle (general fix is right; how to handle the
1 fragile close); she chose **targeted proof**. Swept closers on the failing
`.lean` → Lean's `grind` closes it cleanly; added
`#[verifier::tactus_tactic("intros; grind")]` (transparent, visible, no
vstd-internal names). `_wrong_ensures` soundness pin still correctly fails.

**Discipline notes.** *The probe is the diagnosis.* The DESIGN's own layer-7
characterization (carefully written the night before) named the wrong fix shape;
reading `sst_elaborate` corrected it. *Read the `.lean` after every step* — four
distinct bugs, each only visible in the regenerated output. *The general fix
exposes fragility* — `vec_index` "passed" by luck; converging the forms (the
correct thing) revealed it, which is the fix working, not breaking.

**Review + comprehensive coverage (same session, `4b37ab8` / `23414e7` /
`5f8a8a6`).** Ran the code-review skill (fan-out finder angles + the DESIGN lens
list) on the layer-7 commit. **No live bugs** — the 474-suite already proves no
breakage — but it surfaced five places the code was right *by luck or
convention* rather than by construction, and hardened them: `assert_eq!` on
params↔args / typ_params↔typs (Verus's `sst_elaborate` asserts these; the prior
`zip` truncated silently), an exhaustive `inline_okay` (new `FunctionKind` →
compile error), a broadened `body_blocks_inlining` guard (`AssertBy` /
`OpenInvariant` / `Loop` + any non-empty block, closing the
guard↔`unwrap_trivial_block` mismatch), and a common-case early-out. Then —
Danielle's call — *constructed* cases to try to TRIGGER the documented latent
gaps; all four handled correctly, now pinned: `test_inline_fn_field_access`
(field access on a param inlines, doesn't dangle — it's a projection over a
whole-local read), `_nested_chain` (recursive inline + chain-drop),
`_trait_impl_method` (the `TraitMethodImpl` keep-def branch, previously
unexercised), `_binder_body_kept_folded` (binder body refused → folded → sound).
The field-access pin **disproved a doc comment I'd written in the review commit
an hour earlier** (it claimed field access dangles) — the morning's "checking
catches the careful, confident, wrong thing the last hand left" applied to my
own just-written code. Corrected the comment. 470 → 474 e2e (+4), 268 lib, 0
regressions, vstd 1530/0.

#### Earlier session (2026-05-31 — `HashMap::contains_key` deep-view-body arc: layers 4–6, #122)

**470 e2e (+1), 267 lib (+4), 0 regressions, vstd 1530/0.** Asked to probe the
two residual `contains_key` blockers ("(4) View synthesis + (5) Nonempty") and
look for general, transparent fixes covering both.

**Probe (deliverable: the real layer count).** Generated the `contains_key`
`.lean`, drove its 5 live errors to **0** with surgical edits, isolating the
body's elaboration needs: not two issues but **three** rendering fixes + one
obligation-level deferral. The old "(4) View synthesis / (5) Nonempty" framing
was an *ordering* fix and a *Nonempty-binder* fix, plus a *coercion* fix they
unmasked. Discipline that paid off: a real bug (reading
`DynamicResolved.impl_paths` — empty — instead of the `CallTarget::Fun`
call-level ImplPaths) was caught the instant I read the regenerated `.lean`.

**Design turns (both from Danielle's questions).** (a) I floated axiomatizing
the opaque deep-view def body-less — Danielle: *"don't bodies matter for
proving?"* Right: sound but incomplete (loses Verus `reveal`-parity). **Kept
the body.** (b) My first (4) impl was a slot/clamp early-late insertion —
Danielle: *"that seems a little hacky."* Right: rewrote it as a genuine Kahn's
topological sort (`order_emission`), which fell out cleaner *and* zero-drift.
Each question slid one node into place.

**Layer 6 (`b23e87f`)** — `structural_typ` for `ExprX::Call` returns the
stripped typ, so a ref-decorated call receiver gets its `.mk` (VIR-AST analog
of SST U2). **Layer 4 (`083acae` + `8282da7`)** — `dep_order::order_emission`
topological sort of spec-fn groups + instances; `View (HashMap …)` before the
def that dispatches to it; pure `kahn_emit` unit-tested. **Layer 5
(`06ff961`)** — `nonempty.rs` infers `[Nonempty T]` for choose-over-type-param
via call-graph dataflow; synthetic bound at augment time; pinned same-crate.
**Layer 7 deferred** — the fold/unfold two-renderers divergence (Verus inlines
`Map::contains_key` in exec/SST goals, Tactus folds it in proof/VIR-AST
bc-lemmas; the bc-lemma can't match both); its own convergence arc. Body fully
elaborates; probe 5 → 1. See § "Cross-crate `HashMap::contains_key`
deep-view-body arc" in DESIGN.

**Discipline note.** *Probe-then-build, read the regenerated `.lean` after
every step.* Each layer's fix unmasked the next error beneath it (View
synthesis → caller arity → the missing `.mk` → Nonempty → the obligation);
reasoning forward would have mis-scoped it. And the layer count itself
(3 fixes, not 2) only came from driving the file to zero, not from the prior
note.

#### Earlier session (2026-05-31 — generalized projection-lifting: #122 RC2 partial)

**469 e2e (no count change — `contains_key` still degrades), 263 lib, 0
regressions, vstd 1530/0.** Asked whether RC2 has a "more general thing" too (like
RC4 did), then to build it.

**The general framing (answered + built).** RC2 isn't "fix
`hash_map_deep_view_impl`." Tactus emits assoc types as `outParam`, so ANY `<X as
Trait>::N` projection over an abstract param must be lifted to a fresh binder +
constraint — `impl_subst`'s mechanism, wired to only 2 sites (impl methods,
instance signatures). Generalized it to **every** generic function:
`maybe_augment_standalone_fn` + routing in `generate.rs`'s spec-fn loop. Strict
refinement (empty subst → identity; suite green). Transparency inherits Bug B's
audited verdict (the fresh binder IS the assoc type, pinned by the instance
bracket, visible).

**Two pieces beyond signature-only scope.**
* *Body typ-walk* — `augment_function` rewrites projections in the body (lambda
  binders, call type-args) via a new pub VIR `map_expr_typ_visitor` (the long-flagged
  "public TypX walker for lean_verify"; #94-style visibility promotion).
* *Implicit synthetic binders* — `_tactus_assoc_*` binders render implicit
  (`ASSOC_BINDER_PREFIX` / `is_assoc_binder`) so VIR call sites passing only the
  original typ_args typecheck (Lean infers via the outParam bracket). Caught by the
  probe: after the signature lift, `hash_map_deep_view_impl`'s callers passed 4
  type args against a 6-param signature → `self.deref has type HashMap` mismatch;
  implicit binders cleared it.

**Then option (2) — completed the mechanism, narrowed the residual.** Asked to do
(2) (extend to the broadcast lemma) "then reassess, defer is ok". Each fix peeled
to the next layer and I followed it until the projections were fully gone: (3a)
`augment_function` rewrites require/ensure/returns; (3b) `rewrite_bound` rewrites
projections embedded as trait-bound type-args (`[View Q <K as DeepView>::V]`); (3c)
`generate.rs` applies the augment to broadcast lemmas; (3d) `ImplSubst::build`'s
Source-2 "uncovered slots" pass now actually checks coverage — it was synthesising
a *second* binder for a slot an existing `TypEquality` already pinned, which
`trait_bounds_to_ast` appended as an over-arity `View Q _ta1 _ta2` bracket. With
all four, the entire projection cascade (signature/body/clauses/bounds/broadcast)
is cleared.

**Honest reassessment — residual is now non-projection, deferred.** What's left for
`contains_key` are two semantic issues *inside the `hash_map_deep_view_impl` body*,
separate from projection-lifting: (4) View-instance synthesis (`View (HashMap …) ?V`,
fully polymorphic) and (5) `Classical.epsilon` needs `[Nonempty Key]`. These are
their own sub-arc (elaborating one vstd spec-fn body) — deferred. The
projection-lifting *mechanism* is complete and benefits any future generic
fn/lemma with assoc-type projections; `HashSet::contains` fully verifies (RC4).

**Discipline note.** *The probe-then-build loop kept surfacing the next masked
layer.* Each fix revealed the error beneath the one it fixed — projection-as-accessor
→ caller arity → ensure-clause projection → bound-arg projection → Source-2
over-generation → View synthesis / Nonempty. Reading the regenerated `.lean` after
*every* step (not reasoning forward) is what exposed the stack honestly and told me
when the projection work was genuinely done vs when a green-looking signature still
hid a deeper error.

#### Current session (2026-05-31 — instantiated-param-type call-arg coercion: HashSet::contains verifies, #122 RC4)

**468 → 469 e2e (+1), 263 lean_verify lib, 0 regressions, vstd 1530/0.** Asked
to probe RC2 + RC4 "the right way, general solutions, nothing quick patch", then
to evaluate RC4's transparency before building, then to build it.

**Probe (deliverable: live-confirmed root causes).** Three `lean --json` probes
(`VERUS_KEEP_TEST_DIR=1` → read generated `.lean`): `contains_key` → 6 errors
splitting cleanly into RC2 (DeepView outParam, `473`/`581` + cascade) + RC4
(Box-key, `580` "k.deref : Q" against `Box Q`); `HashSet::contains` → **1 error,
pure RC4** (no DeepView), proving RC2 is map-only and RC4 is shared; incidental:
no `DefaultHasher` error → RC3 confirmed already resolved (the test comment
claiming otherwise was stale). VIR dump (`--log vir-simple`) was decisive: the
`Box::new(*k)` arg is **fully erased** to `ReadPlace(Local k) : TypParam Q` — no
box node survives.

**Transparency eval (Danielle's gate, before building).** The VIR erasure ruled
out a "render the Box::new faithfully" option (nothing to render). Verdict: the
coercion-on-instantiated-type is transparency-clean — faithful reconstruction of
an erased construction, **type-determined** (the call's own explicit typ_arg says
`K=Box<Q>`, not a heuristic), visible in the goal, conservative (wrapper-only
`coerce_lexpr` can't mask inner mismatches), no trust/closer extension. The
*more general* framing chosen over a box-patch: "the call-arg bridge's coercion
target is the callee's param type **with this call's typ_args substituted in**" —
RC4 is the first case, not a special case (altitude lens).

**Build (RC4).** `RenderCtx::fn_param_typs(fun, typ_args)` substitutes
`{typ_params[i] ↦ typ_args[i]}` (`vir::sst_util::subst_typ`) into each param
type, guarded on `typ_args.len() == typ_params.len()` (else declared types →
zero change for concrete calls). Threaded typ_args to all four bridge sites
(`call_to_node`, class-method arm, both SST `CallFun::Fun` arms). Second half:
`proof_fn_signature` rendered req/ensure with `RenderCtx::empty()`, so
broadcast-lemma axioms (the `axiom_*_box` facts) never got the bridge — switched
to `with_fn_map(fn_map)` (it already received `fn_map`; `FnMap` ≡ `RenderFnMap`).
Result: `set.Set.contains (Box Q) m (Tactus.Box.mk k.deref)`. Commits: TBD.

**Discipline notes.**
* *The empty RenderCtx was the hidden half.* After the `fn_param_typs` change the
  set probe STILL showed bare `k.deref` — `fn_param_typs` was returning None
  because `proof_fn_signature` rendered with an empty ctx (no fn_map). Reading the
  regenerated `.lean` (not reasoning) found it; the fix is the one-line
  `empty()` → `with_fn_map(fn_map)`.
* *The good kind of test failure.* The `HashSet::contains` graceful-degradation
  guard I'd added during the probe flipped `Err → Ok` after the fix — the guard
  became a positive verification test (`test_cross_crate_set_contains`).

**Still deferred: RC2 (DeepView outParam) for `contains_key`.** Extend
`impl_subst`'s projection-lifting (fresh `_tactus_assoc_X_V` binder + fake
TypEquality) from impl methods/instance-signatures to the standalone spec fn
`std_specs.hash.hash_map_deep_view_impl` — `ImplSubst::build` is already generic
over `(typ_params, typ_bounds, sig typs)`, so the signature reuses it; the one new
piece is the def's **body** (projection-typed lambda binders `fun (k :
view.DeepView.V Key) => …`), which needs a VIR-AST typ-walk beyond impl_subst's
current signature-only scope.

**Known gap (cheap follow-up):** no isolated unit test on `fn_param_typs`'s
instantiation + arity-guard (fabricating a 26-field synthetic `FunctionX` judged
not worth it vs the faithful e2e guard).
...
**Review + right-way audit (same session, commits `ee8b8ce` / `a0ef39c` /
`6b397c5`; 461 → 465 e2e).** Ran three fresh-eyes reviewers (soundness/
adversarial, correctness/upstream-brittleness, coverage/cleanup) then a
right-way (lens #10) pass; verified every finding myself.
* **Mechanism reviewed sound:** `P` is ∀-bound, so an under-constrained
  prophecy only makes a goal harder, never falsely provable. Two substantive
  fixes: `clear_prophecy` invalidates `P` on resolution (the double-resolve
  `P == P+1` hazard is frontend-unreachable today — `let e = &mut v[i]` is
  rejected as `MutRefCurrent` in an exec body — but now latched by its own
  logic, not the frontend's luck); and reuse restricted to whole-ref
  `MutTargetRaw::Var` targets (a `&mut e.field` of a returned ref would have
  reused the wrong-typed `P`). 4 adversarial soundness pins added
  (`adversarial_probe_{double_bump_named, two_inline_bumps, two_index_bumps,
  unresolved_prophecy}`).
* **Right-way audit (encoding was already right):** the prophecy encoding
  needed no redo (faithful to Verus's `final()`; rewrite-then-subst is the
  established idiom). Two tidiness fixes by the codebase's own conventions:
  removed `MutArgInfo.reused_prophecy: bool` (a `value + bool-which-kind`
  flag-soup — the pattern MutArgInfo/LoopInvKind retired — and *derivable*:
  Phase 1 re-derives reuse as "`fresh` IS the local's registered prophecy");
  and `Option<(LeanName, VarIdent, Typ)>` → named `ReturnProphecy` struct.
  Discipline note: I wrote that flag in the same session I'd read the DESIGN
  typed-invariant section — knowing a pattern by name isn't the same as the
  code embodying it; the audit is what closes that gap.
...
* **`&mut v[i]` (index *mutation*) — bigger than first scoped (spiked
  2026-05-30).** I'd guessed "B1-family, bounded"; a spike disproved it.
  It's a TWO-link chain: **Link 1** was the broadcast wall — vstd's FnMut
  closure axioms (`axiom_fn_mut_call_requires/_ensures`, pulled in by
  default-on-import, irrelevant to indexing) reference a `BuiltinSpecFun`
  that renders as an unresolved literal `builtinSpecFun`. That link is
  now CLEARED (the BuiltinSpecFun broadcast filter landed — see below).
  **Link 2** is the real cost and is NOT bounded: `&mut v[i]` desugars to
  `let e = vec_index_mut(&mut v, i); bump(e);` — a callee that *returns* a
  `&mut`, whose **prophetic** final value the later `bump` sets. The
  verified goal shows `bump`'s `+1` never threading into `v`'s post-state
  (`v`'s update inserts the OLD element value, no `+1`). Tactus's
  new-mut-ref machinery (#107) handles `&mut` *arguments*, not a *returned*
  `&mut` whose prophecy a chained call resolves — that's a genuine
  new-mut-ref feature (returned-mut-ref prophecy composition), a fresh
  arc. Pinned Err by `test_exec_call_mut_arg_vec_index_probe` (now fails
  at Link 2's `tactus_auto failed`, not Link 1's unresolved refs).
...
**BuiltinSpecFun broadcast filter + review-pass cleanup (`2573211`).** The
`&mut v[i]` spike's Link 1 fix, landed standalone (it's a graceful-
degradation improvement on its own): `references_builtin_spec_fun`
(sst_to_lean.rs) skips broadcast lemmas whose require/ensure call a
`BuiltinSpecFun` (closure `call_requires`/`call_ensures`) — the VIR-AST
renderer has no faithful fixed-arity Lean form for them (emits an
unresolved literal `builtinSpecFun`). vstd's `axiom_fn_mut_call_*`, pulled
in by default-on-import but irrelevant to most code, now skip cleanly.
Same family as the un-emittable-trait / FiniteFull filters.

Then a high-effort code review (7 finder angles → verifiers) of the whole
cross-crate View arc. **No correctness bugs** — all three correctness
candidates verified REFUTED, notably the scary "same-crate trait silently
dropped" (disproved by whole-crate-prune analysis: Tactus gets the
whole-crate-pruned krate which force-reaches every same-crate fn, so
`unemittable_traits` only ever flags cross-crate; backed by passing
`test_trait_multi_method`). The review produced a 6-item zero-behavior-
change cleanup (all in `2573211`, 460/0 throughout):
1. **`recv_reads_local` hardened** — peels only transparent wrappers
   (Box/Unbox/CoerceMode/Trigger/CustomErr), not a wildcard over all
   Unary/UnaryOpr. A value-transform between `**self` and `.method()` can
   no longer be mistaken for a pure forward. Unreachable today (the
   opaque-TypParam gate blocks it) but a latent silent-soundness hole if
   ever reached — defense-in-depth for the synth's faithfulness gate,
   mirroring `expr_to_node`'s explicit-transparent-arms structure.
2. **`forwarding_blanket_body` gate** excludes `MutRef` (the synth assumes
   a `Ref Self` `&self` receiver); deref depth is a named const pinned to
   the single-wrapper gate (shape-drift guard).
3. **`unemittable_traits` deduped** to one `expr_shared::unemittable_traits`
   (was byte-identical in generate.rs + sst_to_lean.rs — drift hazard).
4. `Tactus.Ref.mk` wrap uses `expr_shared::apply_wrap_chain` (the
   documented dual of the `apply_deref_chain` on the same line).
5. `references_builtin_spec_fun` uses the read-only `expr_visitor_walk`
   (early `Stop`) instead of `map_expr_visitor` (which clones the tree to
   set a bool).
6. Tidy: re-indented the synth `else` block; hoisted the copy-pasted
   class-emission guard into a `should_emit_class` closure.
