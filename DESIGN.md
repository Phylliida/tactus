...
* **`substitute` boilerplate — LANDED via #98 (2026-05-05).** Four helpers in `lean_ast.rs` concentrate per-variant structural recursion: `walk_children`/`map_children` for ExprNode, `walk_pattern_children`/`map_pattern_children` for Pattern. The five Expr walkers (`substitute_impl`, `collect_free_vars`, `collect_all_names`, `strip_span_marks_node`, `mentions_free_var`) and the two Pattern walkers (`pattern_bound_names_impl`, `rename_in_pattern`) now handle only the variants whose semantics differ from "uniformly recurse" — Var leaves for collectors, Let/Lambda/Forall/Exists/Match for scope-tracking — and delegate everything else via `_ => walk_children(…)` / `_ => map_children(…)`. Each walker shrunk significantly (strip_span_marks_node 70→5 lines, collect_all_names 60→25, collect_free_vars 65→45, substitute_impl 125→75); net file +94 because the helpers themselves are sizeable, but the structural win dominates: walk_children and map_children are exhaustive (no catch-all), so a new ExprNode/Pattern variant is a compile error there, forcing one edit instead of five walker edits. Section-header comment documents the soundness convention for new binder-introducing variants — `substitute_impl`/`collect_free_vars`/`collect_all_names` need explicit arms above the `_ =>` fallthrough so scope tracking stays correct (no compile-time check enforces this; tests that exercise the new variant under substitution catch it). 3 new unit tests pin the helpers' shapes (149 total).
