...
* **`StmX::AssertQuery`** — `assert by(…)` with specific query modes. Three modes exist; their dispositions differ:
  ...
  - `AssertQueryMode::NonLinear` — rejected at this arm. `assert by(nonlinear_arith)` would need a Lean tactic mapping (e.g., `nlinarith`); not wired.
...
* **AssertBitVector with non-trivial SST shapes** (#130 edge) — **real codegen bug, pinned by `test_exec_assert_bit_vector_with_fn_call_panics`** (#147). Probe established BOTH prior claims wrong: (1) Verus does NOT reject upstream — it passes the call through to Tactus; (2) Tactus does NOT produce a clean error — the bit-vector-mode renderer's reject-Call arm IS reached for the BV-mode goal, but Verus's pre-injected `Assume(ens)` (which ast_to_sst inserts before AssertBitVector for Int-mode hyp continuity) goes through the regular Int-mode renderer, which renders `id_u8(x)` as a Var reference. dep_order's VIR-AST walker covers `ExprX::AssertQuery`'s requires/ensures at AST level, but somewhere in the AST→SST→back-to-render path, `id_u8` doesn't reach the preamble's dep set. Result: the sanity check panics in `lean_verify/src/generate.rs:473` with "Tactus codegen produced unresolved references: in `_tactus_ensures_xor_with_call_3`: unresolved `id_u8`." Fix shape (#147): either (a) extend dep_order so the pre-injected Assume's spec fn refs are visible at preamble emission, OR (b) reject Call in the Int-mode pre-injection path too (mirroring the BV-mode rejection), OR (c) confirm Verus's pipeline should never produce this shape and reject earlier.
