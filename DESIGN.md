...
**`Validated` (#100, lifetime dropped via #114) — Exp lowering is panic-free by construction.** Pre-#100, `sst_exp_to_ast(&Exp) -> LExpr` was an "infallible" rendering function whose contract was "caller has already validated via `sst_exp_to_ast_checked`." The contract was a runtime-checked panic; nothing prevented a caller from passing an unvalidated `&Exp`. Post-#100: `Validated` is a newtype constructable only via `Validated::check(&Exp) -> Result<Validated, String>`. The lowering function `lower(&Validated) -> LExpr` cannot panic — the type guarantees the input was already validated. `Wp<'a>` variants (`Let`, `Assert`, `Assume`, `AssertByTactus.cond`, `Branch.cond`, `Loop.cond`/`validated_invs`/`decrease`, `Call.args`) hold `Validated` so the inside of every walker is panic-free by construction. `build_wp` / `build_wp_loop` / `build_wp_call` are where validation happens — the construction sites call `Validated::check(...)?` and propagate the `Err` to callers. The migration shim was removed via #115 (sites use either the typed pipeline or `sst_exp_to_ast_checked(...).expect("<contract>")`). **Lifetime dropped via #114**: pre-#114 was `Validated<'a> { inner: &'a Exp }` — the `'a` tied validation to a borrow into the input SST, conflating "this Exp is valid" (the property) with "this is a borrow into the SST" (a borrow shape). Post-#114, `Validated { inner: Exp }` (`Exp = Arc<...>`, owning is essentially free) — the lifetime parameter goes away entirely. This unblocked build_wp_loop synthesizing a `¬cond_exp` Exp for the non-empty cond_setup transform.
...
* **Non-empty condition setup block — LANDED via #114 sub-feature 1.** Pre-#114 the rejection blocked `while` conditions that Verus's `expr_to_stm_opt` decomposed into setup-stmts + pure expr (notably function calls in cond, which need a temp for the call result). Post-#114, the cond_setup walks as a wp prefix in BOTH the maintain ctx (under `assume cond_exp`) AND the use ctx (under `assume ¬cond_exp`), mirroring Verus's two-query encoding (sst_to_air.rs:2789-2797 + 2730-2737). Setup obligations (e.g., precondition checks for calls inside the cond) emit twice — correct per Verus. **Implementation enabler**: `Validated`'s `'a` lifetime parameter was dropped (now owns its `Arc<Exp>`) so build_wp_loop can synthesize a `¬cond_exp` Exp and wrap it in Validated. **Known automation gap**: when the cond is a function call returning bool, the generated Lean goal has a `_tactus_ret_N : Prop` quantified var with an equality hypothesis; tactus_auto's default closer doesn't substitute the Prop through let-bindings. Users need `#[verifier::tactus_tactic("intros; simp_all; omega")]` or similar — tracked as a follow-up. Pinned by `test_exec_loop_cond_with_setup_no_longer_rejected` (verifies the rejection is gone, not the full closure).
