...
* **Spec fn calls in goal position need explicit unfolding.** The default toolbox handles spec fn calls in *hypothesis* position fine (the hypothesis enters the context as-is and the proof obligation rarely needs to inspect it). In *goal* position — e.g., `id_u8(i+1) == (i+1)` as a loop-invariant maintain step where `id_u8` is a `noncomputable def` — `simp_all` doesn't unfold non-`@[simp]` defs and `decide` doesn't reduce through `noncomputable`. The workaround is `proof { try unfold f }` at the surrounding fn level — pinned by `test_exec_loop_invariant_with_spec_call_try_unfold`. The `try` is load-bearing: Tactus's tactic-prefix mechanism applies the proof block to EVERY theorem the fn emits (init/maintain/use/postcondition), so a bare `unfold f` fails on theorems whose goal doesn't mention `f` (e.g., the first invariant init theorem proving just `i ≤ n`). `try unfold f` no-ops where `f` doesn't appear and unfolds where it does. Per-fn override (`#[verifier::tactus_tactic("(try unfold id_u8); tactus_auto")]`) works the same way for the same reason. Surfaced 2026-05-09 review pass for #147; bare-`unfold` shape was an incomplete guess corrected 2026-05-10 by probing what actually closes the goal.
