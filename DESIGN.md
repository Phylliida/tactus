...
### Tuple-destructure asserts — don't intro the lets (2026-06-07)
...
The honest fix is **subtraction** — don't inject the breaking `intro` in the first place. omega's *own* zeta reduces *goal-position* lets fine (verified: bare `omega` closes the chain with no intro). The intro existed only for Binder/Hyp frames that need source names (loop locals — BUG-loop-local-names); for the all-`Let` tuple shape there are no names to expose, so `emit_with_closer` skips it and the user's `by { omega }` runs on the real goal — generated Lean is literally `omega`. (Honest cost: `by { intros; omega }` still fails, because the user's *own* `intros` creates the ldecls — but that's their tactic, visibly, not Tactus hiding a step.) Pinned by `test_exec_tuple_destructure_assert_omega`; landed `fd557a7`, superseding the rejected simp-injection `4ed848e`.
