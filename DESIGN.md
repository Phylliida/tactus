...
* **`BinaryOp::Xor`** — renders via the shared `non_binop_head(Xor) -> "Bool.xor"`. Pinned at three levels: `test_exec_xor_bool` (fn-return-equals-body smoke), `test_exec_xor_bool_concrete` (concrete `(true ^ false) == true` closes via `decide`), `test_exec_xor_bool_free_vars_commutative_gap` (commutativity on free bool vars FAILS — pinned as `Err`). The free-vars gap surfaces two compounding limitations: (a) `to_lean_type::typ_to_node` always emits `Prop` for `TypX::Bool` regardless of context, so exec-bool params become `Prop` and Lean inserts `decide` coercions when the value flows into `Bool.xor`; (b) `Bool.xor_comm` isn't in `tactus_auto`'s simp set, so the resulting `(decide b1 ^^ decide b2) = (decide b2 ^^ decide b1)` doesn't close. Workaround: `assert(... == ...) by { simp_all [Bool.xor_comm] }`. Proper fixes either land context-sensitive bool rendering (the "Bool vs Prop" design intent under "Bool vs Prop" above) or extend the closer's simp set; both are task-sized.
