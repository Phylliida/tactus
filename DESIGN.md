...
* **Generic recursive datatype with cross-instantiation recursion** (#108 edge). `enum Mut<A> { Plain(A), Recurse(Mut<u8>) }` — the recursive arm has type `Mut<u8>`, not `Mut<A>`. `field_recursive_target` (renamed from `field_is_self_recursive` per #109) checks SCC-path membership regardless of args, so it correctly identifies the recursion; `T.height` is generic over the type-arg, so `Mut.height rest` (with `rest : Mut u8`) elaborates via Lean's implicit inference. Should work but no test exercises this specific shape.
...
* **Datatype SCCs of size > 3** (#109 edge). 3-element SCCs are tested via `test_exec_three_element_datatype_scc`; 4+ element cycles go through the same Tarjan + mutual-block code path but lack explicit regression tests. Should work — the Tarjan implementation is generic over SCC size — but unverified for very deep cycles where Lean's mutual-block compilation cost might surface.
...
* **AssertBitVector with non-trivial SST shapes** (#130 edge). `sst_exp_to_bit_vector_ast` handles `Var` / `Const` / `BinaryOp` / `UnaryOp::Not` — the shapes a typical `assert(P) by(bit_vector)` uses. Function calls, struct constructors, lambdas inside the assertion are rejected with a clear error suggesting `assert(P) by { ... }` with a custom Lean tactic instead. Verus typically rejects most of these upstream too, so the rejection is theoretical; no test exercises it.
