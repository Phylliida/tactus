...
* **Datatype SCCs of size > 3** (#109 edge) — ✅ pinned by `test_exec_four_element_datatype_scc` (4-cycle: A → B → C → D → A) and `test_exec_five_element_datatype_scc` (5-cycle: P → Q → R → S → T → P). The Tarjan implementation is generic over SCC size; 4+ cycles go through the same `mutual { inductives } end` + accessors-out + `mutual { heights } end` emission path. Very deep cycles (e.g., 10+) remain unpinned — Lean's mutual-block compilation cost is the latent concern at extreme depth — but the structural correctness is confirmed end-to-end at depths 4 and 5.
