...
* **Datatype SCCs of size > 3** (#109 edge) — ✅ pinned by `test_exec_four_element_datatype_scc` (4-cycle: A → B → C → D → A). The Tarjan implementation is generic over SCC size; 4+ cycles go through the same `mutual { inductives } end` + accessors-out + `mutual { heights } end` emission path. Very deep cycles (e.g., 10+) remain unpinned — Lean's mutual-block compilation cost is the latent concern at extreme depth — but the structural correctness is now confirmed end-to-end at depth 4.
