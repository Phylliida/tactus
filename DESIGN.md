...
* **`StmX::AssertBitVector` — LANDED via #111.** `assert(...) by(bit_vector) requires P; ensures Q;` routes through `Wp::AssertBitVector { req_conj, ens_conj, rust_loc, body }`. The walker emits one theorem with goal `req_conj → ens_conj` (collapsed to bare `ens_conj` when requires is empty) and uses the prelude tactic `tactus_bit_vector` as the closer. The proven `ens_conj` then enters the body's ctx as a Hyp, matching Verus's "publish ensures to the surrounding context" semantics. The tactic is a best-effort ladder (`decide` / `simp_all + omega` under `intros`) because Tactus's u-types render as `Int` rather than `BitVec` — Lean's full SAT-backed `bv_decide` doesn't apply directly. Users needing richer reasoning override per-fn via `#[verifier::tactus_tactic("…")]` or fall back to `assert(P) by { … }` with a custom tactic. Pinned by `test_exec_assert_bit_vector_concrete` (positive: `5 ^ 3 == 6` closes via `decide`) and `test_exec_assert_bit_vector_false` (negative: `x ^ x == 1` fails with `tactus_bit_vector: could not discharge`).
