...
* **Spec fn calls in goal position need explicit unfolding.** The default toolbox handles spec fn calls in *hypothesis* position fine (the hypothesis enters the context as-is and the proof obligation rarely needs to inspect it). In *goal* position — e.g., `id_u8(i+1) == (i+1)` as a loop-invariant maintain step where `id_u8` is a `noncomputable def` — `simp_all` doesn't unfold non-`@[simp]` defs and `decide` doesn't reduce through `noncomputable`. Users need a `proof { unfold id_u8 }` block at the surrounding fn level, or a per-fn override that includes `unfold` in its tactic. Surfaced 2026-05-09 review pass for #147 when probing spec fn calls in loop invariants. The fix surface for #147 covers the *codegen* (spec fn def reaches preamble); this is the next layer down.
