...
* **`StmX::AssertQuery`** — `assert by(…)` with specific query modes. Three modes exist; their dispositions differ:
  ...
  - `AssertQueryMode::NonLinear` — LANDED. Lowers to `Wp::AssertQuery { closer: "first | (intros; nlinarith) | tactus_auto", preamble: [Mathlib.Tactic.Linarith], body, after }`. The walker enters a new OblCtx scope via `obl.new_scope(closer, preamble)` which drops enclosing-scope Hyps (matching Verus's NonLinear query semantics — only requires + typ invariants are in scope) and installs the closer + preamble on the obl. Every theorem the body's recursive walk emits picks them up. Generic over closer name — future query modes (Polyrith etc.) would reuse `Wp::AssertQuery` with a different closer string. Pinned by `test_exec_assert_nonlinear_commutative`, `_with_requires`, `_with_proof_block`, `_wrong` (negative).
