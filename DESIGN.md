...
* **Non-empty condition setup block — LANDED via #114 sub-feature 1.** Pre-#114 the rejection blocked `while` conditions that Verus's `expr_to_stm_opt` decomposed into setup-stmts + pure expr (notably function calls in cond, which need a temp for the call result). Post-#114, the cond_setup walks as a wp prefix in BOTH the maintain ctx (under `assume cond_exp` via `Wp::Assume`) AND the use ctx (under `assume ¬cond_exp` via `Wp::Hyp`), mirroring Verus's two-query encoding (sst_to_air.rs:2789-2797 + 2730-2737). Setup obligations (e.g., precondition checks for calls inside the cond) emit twice — correct per Verus. **Implementation note**: the negated cond goes through a new `Wp::Hyp { hyp: LExpr, body }` variant for already-rendered hypotheses, rather than synthesizing a fresh `¬cond_exp` Exp. The Wp::Hyp variant is the right shape because the negation is derived (not borrowed from the input SST); building it at LExpr level avoids the lifetime/Arc juggling that synthesizing an SST Exp would require. The Validated/Wp::Assume contract stays scoped to genuine SST borrows. **Known automation gap**: when the cond is a function call returning bool, the generated Lean goal has a `_tactus_ret_N : Prop` quantified var with an equality hypothesis (`_tactus_ret_N = (x > 0)`) followed by nested let-bindings and implications. tactus_auto's default closer doesn't substitute the Prop through let-bindings — `omega` rejects the ∀-Prop quantifier, `simp_all` doesn't intro outer ∀s. Users need `#[verifier::tactus_tactic("intros; simp_all; omega")]` or similar override — but even the override doesn't fully close all obligations (the test name `_no_longer_rejected` reflects this; it only confirms the up-front rejection is gone). Tracked as #128. Probed 2026-05-04: a tactic-level fix isn't sufficient — adding `(intros; simp_all; first | omega | rfl | trivial)` alternatives to tactus_auto fails because `simp_all` errors on no-progress and omega doesn't see the let-bindings as hypotheses. The real fix is encoding-level: when the callee's ensures has the form `ret == E`, codegen should substitute E for the ∀-bound ret directly so the goal never has the awkward shape. That's a more involved change to `walk_call`'s spec inlining and is what #128 needs.
