...
* **`RewritePipeline` for SST→SST passes.** Tactus runs several SST→SST rewrite passes inside `exec_fn_theorems_to_ast` (in `sst_to_lean.rs`): `normalize_mut_ref_in_stm` (#95) maps new-mut-ref shapes back to legacy form; `rewrite_varat_for_mut_params_in_stm` (#94) renames pre-state references; `is_synthetic_assume_to_drop` filters out Verus-internal `Assume(HasResolved(...))` / closure-spec assumes during walk; `collect_borrow_mut_links` + `is_borrow_mut_linkage_assign` (2026-05-26) eliminate Verus's BorrowMut indirection in caller-side new-mut-ref calls — see "BorrowMut elimination" entry below. They compose by being called in sequence in the orchestrator (so the order is implicit in the Rust source).

  (Nat-coercion insertion used to be a fifth pass here; as of 2026-06-05 it's a gated Verus `Clip` instead — see § "U → Nat coercion".) A typed pipeline — e.g., `RewritePipeline::new().drop_synthetic_assumes().normalize_mut_ref(&params).rewrite_varat(&params).eliminate_borrow_mut().run(stm)` — would make the data flow explicit, let new passes be added without touching the orchestrator, and surface ordering invariants (e.g., "normalize before rewrite") as compile errors when a new pass is inserted in the wrong place.
...
* **`&mut` args** — caller-side LANDED (#55, slice 1). `walk_call`
  ...
  - **Caller side LANDED in slice 1; callee side LANDED via #94.**
    ...
    * `&mut v[i]` (Index L-value) — **LANDED 2026-05-30** via
      returned-mut-ref prophecy composition (a bounded #107 sibling; full
      mechanism in § "Returned-mut-ref prophecy composition"). `vec_index_mut`'s
      returned `&mut`'s `*final` is threaded as a prophecy var `P` and unified
      with the resolving `bump` call. Pinned by `test_exec_call_mut_arg_vec_index`
      (+ `_wrong_ensures`).
...
### U → Nat coercion — gated Verus `Clip` for the Lean backend (BUG-as-nat-cast.md + BUG-ch5-pow-iter F2; reshaped from a Tactus pre-pass to a Verus gate 2026-06-05)

> **Cast-rendering policy is recorded in [`DECISION-cast-rendering.md`](DECISION-cast-rendering.md):** keep `nat → Nat` (not Option B's `nat → Int`); don't add an auto cast-folding closer rung (Option A); make `as nat` render **consistently** as `ℕ`. That policy is unchanged — only the *implementation* moved (2026-06-05) from a Tactus-side pre-pass to a gated Verus change.

**The problem.** Verus's `fn_call_to_vir.rs` cast lowering drops `U(_) → Nat` and `USize → Nat` casts as no-ops (`Ok(source_vir)`). Sound for Z3 (u_N and `nat` are both `Int` with refinements); wrong for Lean, where Tactus renders `u_N` as `Int` and `nat` as `Nat` (distinct types), so `f(i as nat)` for `i : u64` lowered to `f i` with `i : Int` → fails Lean's type check.

**The fix (2026-06-05): gate the cast at its source.** A `--lean-backend` config flag (`cmd_line_args.lean_backend`) makes the two cast-drop arms in `fn_call_to_vir.rs` emit `mk_ty_clip(..., Nat)` instead of dropping — so `x as nat` becomes `Clip{Nat}` (rendered `Int.toNat x`) directly in the VIR Verus produces, on **every** `as nat` site uniformly. Default off → the Z3 path is byte-identical to upstream Verus. ~20 lines across `config.rs` (the flag, mirroring `--emit-lean`) + `fn_call_to_vir.rs` (the gated arm).

This **replaced** the prior Tactus-side `insert_nat_coercions_in_{exp,stm,expr}` pre-pass — a ~200-line `nat_coercion.rs` module (now deleted) that re-materialized the Clip at fn entry, *after* Verus dropped it, by looking up each call's callee in `fn_map` and wrapping Int-rendering args bound for Nat params. The gate subsumes the whole pre-pass — *including* the "Friction 2" arith-operand consistency case (`(x as nat) * f`, which the pre-pass had a dedicated `BinaryOp::Arith` arm for). Because every `as nat` now simply keeps its `Clip`, operands, call-args, *and even comparisons* are all consistently `ℕ` — strictly more uniform than the pre-pass, which only handled call-args + arith-operands and deliberately left comparisons mixed.

**Why a gate, not an always-on Verus change.** Always-emitting Clip globally breaks 7 vstd bit-shift lemmas (`vstd/bits.rs`: `lemma_*_shr_is_div`, `lemma_*_shl_is_mul`) whose calc-style Z3 proofs rely on Z3 silently equating `x` and `clip(Nat, x)`. The gate keeps those off the flag: **vstd is built without `--lean-backend`** (Z3, casts dropped, 1530/0 preserved); Lean-targeted code is built with it.

**Why per-crate (a flag), not per-fn.** For exec/proof fns we *do* know the target per-fn (the `tactus_auto` / tactic-body attr, readable at the cast site via `bctx.fun_id`). But the cast also appears in **spec-fn bodies** (`spec fn f(x: u64) -> nat { x as nat }`) — shared *definitions* with no per-fn target; whether they render to Lean depends on the caller, and they are lowered once before any caller is known. So the real granularity is **per-crate**: a Tactus crate renders entirely to Lean; vstd is the Z3 crate. `--lean-backend` reads as "*this crate targets Lean,*" which covers spec fns correctly where per-fn detection can't. (Doing per-fn *after* VIR is literally the pre-pass we deleted — the win was moving the decision to lowering time, where per-crate is the natural unit.)
...
**Block language follows routing ("flag decides", 2026-07-02).** Under `--lean-backend`, a Lean-routed exec fn's `proof { … }` / `assert(..) by { … }` blocks ARE Lean tactic text — no attribute needed. The FileLoader carries the flag (`TactusFileLoader { lean_backend }`, threaded into both compiler-callback constructions so the verification and compile passes sanitize identically) and pass 2 sanitizes blocks inside `#[verifier::tactus_auto]` fns OR, under the flag, inside plain exec fns (`is_lean_routed_exec_fn`: no `proof`/`spec` mode keyword, no `#[verifier::z3]` attr — the textual mirror of the routing rule). The HIR-side TactusSpan gate (`enclosing_fn_has_lean_tactic_blocks`) applies the same rule from `bctx.mode` + `cmd_line_args.lean_backend` + attrs; the FileLoader and HIR gates agreeing is the load-bearing consistency requirement. `#[verifier::z3]` therefore opts a fn out of BOTH: it routes to Z3 and its blocks are Verus ghost code (pinned by `test_lean_backend_z3_optout_verus_proof_block`; the attr-less directions by `test_lean_backend_execfn_lean_proof_block_no_attr` / `_lean_assert_by_no_attr`).

History: the previous semantics let attr-less Lean-routed fns carry **Verus** ghost blocks (WP-translated as SST statements) — tactus-group-theory's `runtime.rs`/`todd_coxeter_rt.rs` used this. When retirement-by-detection was scoped (2026-07-02), that per-fn content-language ambiguity was the blocker; the owner resolved it by making the flag decide, and those crates' 12 block-carrying exec fns were `#[verifier::z3]`-marked as the migration interim (drop the attr when a fn's blocks are ported to Lean tactics). `tactus_auto` remains accepted — redundant under the flag, still the per-fn opt-in for non-lean-backend crates.
...
* **`&mut` args on calls (#55) — caller-side LANDED.**
  ...
  **Explicit deferrals (still rejected in `build_wp_call`):**
  - **`&mut v[i]`** (Index L-value) — **LANDED 2026-05-30** via
    returned-mut-ref prophecy composition (a bounded #107 sibling; see §
    "Returned-mut-ref prophecy composition" for the mechanism). The
    `&mut v` arg is Var-shaped (`MutTargetRaw::Var`), the rebind is
    Lean-native (`Seq::update`), and the returned `&mut`'s `*final` is
    threaded as a prophecy var `P` unified with the resolving `bump` call.
    In legacy mode `&mut v[i]` is still upstream-blocked outright ("index
    for &mut not supported", `rust_to_vir_expr.rs:3284`) — the landing is
    new-mut-ref mode. Pinned by `test_exec_call_mut_arg_vec_index`
    (+ `_wrong_ensures`).
...
over an *abstract* type-param can't render as an accessor — it must be lifted to
a fresh type-param binder constrained via the trait's instance bracket (the Bug-B
mechanism, § "Blanket-impl assoc-type passthrough"). That lift was wired to only
**two** emission sites: impl-method standalone defs and instance signatures. This
landing makes it a **uniform property of every emitted generic function**, the
direct analog of RC4's "instantiated param types" move — generalize the mechanism,
don't special-case the one fn that tripped it.

**The mechanism was already generic; only its wiring was narrow.**
`ImplSubst::build(typ_params, typ_bounds, sig_typs, traits)` discovers projections
(Source 1) + bound-trait assoc-type slots (Source 2) and produces fresh binders +
fake `TypEquality` bounds + a proj_map — none of it impl-specific. New
`impl_subst::maybe_augment_standalone_fn(f, traits)` drives it from a *standalone*
fn's own `typ_params`/`typ_bounds`, and `generate.rs`'s spec-fn emission loop now
routes non-`TraitMethodImpl` fns through it (TraitMethodImpl keeps the per-impl
subst with its method_context). Strict refinement: a fn with no projections →
empty subst → identity (the full 469-test suite stays green).

**Two pieces beyond the prior signature-only scope:**
1. **Body typ-walk.** `augment_function` previously rewrote only param/ret typs.
   A standalone spec fn like `hash_map_deep_view_impl` also carries projections in
   its *body* — lambda binder annotations (`fun (k : DeepView.V Key) => …`) and
   `Map.new (DeepView.V Key) …` type-args. `augment_function` now applies
   `rewrite_typ_rec` to every Typ embedded in the body via a new pub VIR helper
   `vir::ast_visitor::map_expr_typ_visitor` (mirrors `map_expr_visitor` /
   `map_expr_place_visitor` but threads the typ callback — the "public TypX walker
   for lean_verify" DESIGN flagged as future infra; #94 precedent for promoting a
   VIR visitor's visibility for the fork). No-op when proj_map is empty.
2. **Implicit synthetic binders.** The fresh `_tactus_assoc_X_Trait_N` binders MUST
   render *implicit*, not explicit: they're determined by their trait's outParam
   instance bracket, so Lean infers them, and VIR-rendered call sites (which pass
   only the original typ_args, knowing nothing of the synthetic params) would
   otherwise fail with an arity mismatch — concretely, `hash_map_deep_view_impl`'s
   callers (`HashMap.DeepView.impl.deep_view` body + the DeepView instance) pass 4
   type args against a now-6-param signature. `fn_binders` renders any typ_param
...
instance bracket; visible in the generated Lean; the alternative (V-as-field) was
...
`&mut v[i]` (index mutation) verifies. The last blocker was the RETURNED
`&mut` from `vec_index_mut`. **Pinned by `test_exec_call_mut_arg_vec_index`
(Ok) + `test_exec_call_mut_arg_vec_index_wrong_ensures` (Err soundness pin —
`+2` fails, not via a vacuous hypothesis).**

**What landed** (the mechanism below was the scoped plan; it landed verbatim,
a bounded #107 sibling in `sst_to_lean.rs`):
* `OblCtx.prophecies` (caller-local → prophecy var `P`), keyed
  disambiguator-aware via `borrow_mut_key`, flowing with the walk.
* `push_post_call_frames`: when a call's dest is `MutRef`-typed, mint `P`
  (∀-bound at the `MutRef T` wrapper typ + inner bound), render the callee's
...
* **Transparency held by construction:** GENERAL over any `MutRef`-typed dest
  (no `vec_index_mut` name-check); faithful to Verus's `final()` semantics
...
* **Indexed L-values (`&mut v[i]`)** — LANDED 2026-05-30. The *rebind/update* shape is Lean-native (no `Vector.set`, no havoc): Verus desugars `&mut v[i]` to `vec_index_mut(&mut v, i)`, whose spec `final(vec)@ == old(vec)@.update(i, *final(element))` makes `Seq::update` carry the "index j≠i unchanged" property structurally. The one Tactus-side piece — `*final(element)` is a returned-mut-ref *prophecy* — is the returned-mut-ref prophecy composition (§ "Returned-mut-ref prophecy composition"): mint a prophecy var `P`, render `*final(element)` as `P`, unify `P` with the resolving `bump` call's post-state. A good example of the discipline: the SMT-style "havoc + index-unchanged conjunction" sketch was wrong; `Seq.update` + a single prophecy variable is the Lean-native shape.
...
### Assert-by intro discipline — Tactus intros only to name a Binder (2026-06-07)

A third point on the transparency thread, and the one that distinguishes *neutral scaffolding* from *hidden reasoning*. `let (a, b) = call(..)` lowers to a tuple-typed let-alias chain (`let tmp := ret; let a := tmp.1; let b := tmp.2`). For `assert(P) by { tac }`, `emit_with_closer` used to inject `intro <let-names>;` before the user's tactic — and intro-ing a *tuple-typed* let turns it into an opaque `ldecl` that omega can't push the projection through, so `assert(b <= K) by { omega }` couldn't reach the call's inlined `ensures` (stated over `ret`). Blocked tuple-returning recursive exec fns (BUG-tuple-destructure-alias-temps-block-omega.md).

The tempting fix was the § "Self-assignment snapshot temps" move — absorb it downstream by injecting `simp only [<lets>]` after the intro (Tactus generates the names, so it has them). But that fails transparency *from the other direction*: a `simp` the user didn't write is hidden goal-rewriting ("and then a simp happened"), the very anti-pattern principle #1 forbids in the closer. `intro` is neutral scaffolding (it moves binders into context without changing what's true); `simp` is reasoning. The line is there.

The honest fix is **subtraction**, and it resolves to a single rule: **`emit_with_closer` injects `intro <names>;` ONLY when `remaining` carries a `Binder` frame** — a loop local blocked behind a Let that would otherwise get an inaccessible `i✝` dagger name (BUG-loop-local-names). `Let` frames (synthetic temps the user never names) and `Hyp` frames (anonymous `→` antecedents) are NOT intro'd: omega's own zeta reduces goal-position lets, and omega/simp_all intro `→` themselves.

**The consequence is a clean, general contract: the user tactic owns its own intro.** Intro-aware tactics (`omega`, `simp_all`) need nothing — `assert(b <= K) by { omega }` runs on the real goal, generated Lean is literally `omega`. Non-intro-aware Mathlib tactics (`nlinarith` / `linarith` / `ring` — they act on the *current* goal, not goal-position binders/lets/hyps) write `by { intros; nlinarith }`, the idiom DESIGN already documented under "tactic / automation limitations". This was chosen (option **E**) over a type-aware "intro all *but* tuple-typed lets" gate (**A**): a general rule beats a special case (altitude lens), and the user's `intros` is *visible in their tactic*, not a hidden Tactus step. (Honest costs, both visible-not-hidden: `by { intros; omega }` fails — the user's own `intros` makes the ldecls; and `by { nlinarith }` now needs `by { intros; nlinarith }`.) Pinned by the `test_exec_tuple_destructure_*` battery (omega / simp_all / default-closer / both-elements / triple / construction-postcondition / loop-interaction + wrong-bound and user-intros-fails negatives) and `test_self_assign_mul_overflow_bound` (the `intros; nlinarith` idiom). Superseded the rejected simp-injection `4ed848e`.

**The three moves, in order of preference.** (1) *Don't create the friction* — if a Tactus-added step (here, the `intro`) is what breaks the downstream tactic, remove the step. (2) *Render faithfully + absorb uniformly* — when the artifact is Verus's and removing it needs escape analysis, keep it and read through it with one closer rung (§ "Self-assignment snapshot temps"). (3) *Inject hidden reasoning* — never. Removing an SMT encoding outright when Lean's types make the property structural (§ "What doesn't have to mirror Verus's encoding") is a fourth, orthogonal move: available only when the property is genuinely structural, not merely cosmetic.
