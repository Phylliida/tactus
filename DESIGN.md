...
* **Spec fn calls in goal position need explicit unfolding.** The default toolbox handles spec fn calls in *hypothesis* position fine (the hypothesis enters the context as-is and the proof obligation rarely needs to inspect it). In *goal* position — e.g., `id_u8(i+1) == (i+1)` as a loop-invariant maintain step where `id_u8` is a `noncomputable def` — `simp_all` doesn't unfold non-`@[simp]` defs and `decide` doesn't reduce through `noncomputable`. **Two workarounds, both pinned 2026-05-10:**
  ...
  **Future work (#148): `invariant P by { tac },` syntax.** A more direct surface — proof attachment right at the invariant clause — was prototyped 2026-05-10. Stage 0 (parser change in syn-verus, `Specification.tactics` parallel array capturing `by { tac }` after each spec expr) landed and stays as a readiness footprint with its sanity tests (`test_exec_invariant_by_tac_parses_stage0`, `_mixed`). Stages 1-2 (proc-macro desugaring → body-assert + codegen wiring) attempted but rolled back: the desugaring approach hit a parser interaction we didn't fully diagnose, and the cleaner pipeline-threading alternative required ~150-250 lines across VIR/SST/codegen — more cost than the readability win warranted. If this comes up again, the natural restart point is Stage 1's desugaring with careful debugging of the parser interaction (the fundamental design seems right; implementation snag).
