...
theorem lib.amended_shapes_kernel_compute :
  ...
  decide
theorem _tactus_postcondition_u_esf_deadend_at_lib_3608_13_1 (hp : Int → (Int → Int) → Prop) (he : lib.ExprData → (Int → Int) → Prop) (lv : Int → (Int → Int) → Int) (f : lib.FrameList) (b : Tactus.Box lib.StmData) :
    /- @rust:lib.rs:3608:13 -/ ∀ (st : Int → Int), lib.exec_safe_f hp he lv f (lib.StmData.DeadEnd b) st = lib.exec_safe_f hp he lv f b.deref st := by
