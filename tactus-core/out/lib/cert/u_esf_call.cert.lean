...
-- tactus certificate (stage A) — crate `lib`, fn `u_esf_call`
...
-- leaf 12: ⟦post⟧
-- leaf 13: ⟦Tactus.Box lib.FrameList⟧
-- leaf 14: ⟦∀ (st : Int → Int), lib.exec_safe_f pp hp he lv f (lib.StmData.Call reqs post) st = lib.close_sem_obligs pp hp he lv f st reqs⟧
-- leaf 15: ⟦/- @rust:tactus-core/lib.rs:4939:13 -/ ∀ (st : Int → Int), lib.exec_safe_f pp hp he lv f (lib.StmData.Call reqs post) st = lib.close_sem_obligs pp hp he lv f st reqs.deref⟧

@[reducible] def cert_u_esf_call_ctx : lib.FnCtxData :=
...
@[reducible] def cert_u_esf_call_sst : lib.StmData :=
...
example : lib.stm_size cert_u_esf_call_sst = 2 := by decide

-- ── production goals (N3b) ──────────────────────────────────
-- goal 0: _tactus_postcondition_u_esf_call_at_lib_4939_13_1
@[reducible] def cert_u_esf_call_goals : lib.GoalList :=
...
example : lib.goal_count cert_u_esf_call_goals = 1 := by decide
