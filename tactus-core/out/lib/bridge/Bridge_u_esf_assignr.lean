...
-- tactus certificate (stage A) — crate `lib`, fn `u_esf_assignr`
...
-- leaf 13: ⟦h_x_bound⟧
-- leaf 14: ⟦v⟧
-- leaf 15: ⟦0 ≤ v ∧ v < 18446744073709551616⟧
-- leaf 16: ⟦h_v_bound⟧
-- leaf 17: ⟦∀ (st : Int → Int), lib.exec_safe_f pp hp he lv f (lib.StmData.AssignR x v) st = True⟧
-- leaf 18: ⟦/- @rust:tactus-core/lib.rs:4935:13 -/ ∀ (st : Int → Int), lib.exec_safe_f pp hp he lv f (lib.StmData.AssignR x v) st = True⟧

@[reducible] def cert_u_esf_assignr_ctx : lib.FnCtxData :=
...
@[reducible] def cert_u_esf_assignr_sst : lib.StmData :=
...
example : lib.stm_size cert_u_esf_assignr_sst = 2 := by decide

-- ── production goals (N3b) ──────────────────────────────────
-- goal 0: _tactus_postcondition_u_esf_assignr_at_lib_4935_13_1
@[reducible] def cert_u_esf_assignr_goals : lib.GoalList :=
...
example : lib.goal_count cert_u_esf_assignr_goals = 1 := by decide

-- ── W4a in-gate bridge (bootstrap-38) ──
set_option maxRecDepth 8000
example : lib.goals_eq (lib.ref_wp cert_u_esf_assignr_ctx cert_u_esf_assignr_sst) cert_u_esf_assignr_goals = 1 := by decide
