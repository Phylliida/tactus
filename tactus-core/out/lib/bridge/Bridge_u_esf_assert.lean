...
-- tactus certificate (stage A) — crate `lib`, fn `u_esf_assert`
...
-- leaf 18: ⟦h_h_bound⟧
-- leaf 19: ⟦∀ (st : Int → Int), lib.exec_safe_f pp hp he lv f (lib.StmData.Assert o hn h) st = lib.close_sem_e pp hp he lv f st o⟧
-- leaf 20: ⟦/- @rust:tactus-core/lib.rs:4918:13 -/ ∀ (st : Int → Int), lib.exec_safe_f pp hp he lv f (lib.StmData.Assert o hn h) st = lib.close_sem_e pp hp he lv f st o⟧

@[reducible] def cert_u_esf_assert_ctx : lib.FnCtxData :=
...
@[reducible] def cert_u_esf_assert_sst : lib.StmData :=
...
example : lib.stm_size cert_u_esf_assert_sst = 2 := by decide

-- ── production goals (N3b) ──────────────────────────────────
-- goal 0: _tactus_postcondition_u_esf_assert_at_lib_4918_13_1
@[reducible] def cert_u_esf_assert_goals : lib.GoalList :=
...
example : lib.goal_count cert_u_esf_assert_goals = 1 := by decide

-- ── W4a in-gate bridge (bootstrap-38) ──
set_option maxRecDepth 8000
example : lib.goals_eq (lib.ref_wp cert_u_esf_assert_ctx cert_u_esf_assert_sst) cert_u_esf_assert_goals = 1 := by decide
