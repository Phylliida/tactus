...
theorem lib.amended_shapes_kernel_compute :
  ...
  decide
theorem _tactus_postcondition_u_cso_hoist_mode_at_lib_3533_13_1 (hp : Int → (Int → Int) → Prop) (he : lib.ExprData → (Int → Int) → Prop) (lv : Int → (Int → Int) → Int) (f : lib.FrameList) (l : lib.RawExpList) (h_req0 : ¬(lib.has_plain_flet f = 1)) :
    /- @rust:lib.rs:3533:13 -/ ∀ (st : Int → Int), lib.close_sem_obligs hp he lv f st l = lib.close_sem_obligs_hoist hp he lv f st l := by
  first | tactus_auto | (intros <;> simp_all [close_sem_obligs])
