...
theorem lib.amended_shapes_kernel_compute :
  ...
  decide
theorem _tactus_postcondition_u_esf_call_at_lib_3603_13_1 (hp : Int → (Int → Int) → Prop) (he : lib.ExprData → (Int → Int) → Prop) (lv : Int → (Int → Int) → Int) (f : lib.FrameList) (reqs : Tactus.Box lib.RawExpList) (post : Tactus.Box lib.FrameList) :
    /- @rust:lib.rs:3603:13 -/ ∀ (st : Int → Int), lib.exec_safe_f hp he lv f (lib.StmData.Call reqs post) st = lib.close_sem_obligs hp he lv f st reqs.deref := by
