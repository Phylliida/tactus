...
theorem lib.amended_shapes_kernel_compute :
  ...
  decide
theorem _tactus_postcondition_u_esf_ret_at_lib_3617_13_1 (hp : Int → (Int → Int) → Prop) (he : lib.ExprData → (Int → Int) → Prop) (lv : Int → (Int → Int) → Int) (f : lib.FrameList) (es : Tactus.Box lib.RawExpList) (rb : lib.RetBind) :
    /- @rust:lib.rs:3617:13 -/ ∀ (st : Int → Int), lib.exec_safe_f hp he lv f (lib.StmData.Ret es rb) st = lib.close_sem_obligs hp he lv (lib.ret_frame f rb) st es.deref := by
