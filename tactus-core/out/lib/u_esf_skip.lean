...
theorem lib.amended_shapes_kernel_compute :
  ...
  decide
theorem _tactus_postcondition_u_esf_skip_at_lib_3648_13_1 (hp : Int → (Int → Int) → Prop) (he : lib.ExprData → (Int → Int) → Prop) (lv : Int → (Int → Int) → Int) (f : lib.FrameList) :
    /- @rust:lib.rs:3648:13 -/ ∀ (st : Int → Int), lib.exec_safe_f hp he lv f lib.StmData.Skip st = True := by
