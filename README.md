...
```bash
LEAN_PATH="$HOME/.cache/tactus/prelude:$LEAN_PATH" lean --json path/to/fn.lean
```

(Override the cache root with `$TACTUS_PRELUDE_CACHE`; `tactus-lsp` and
the batch verifier do this wiring automatically. The cache rebuilds in
place when the prelude changes — artifacts from an older prelude fail
and are regenerated by re-running tactus.)
