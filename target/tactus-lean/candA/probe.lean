...
macro_rules
  | `(tactic| tactus_peel) => `(tactic|
      first
        ...
        | skip)
namespace candA
noncomputable def sq (n : Nat) : Nat :=
  n * n
theorem _tactus_assert_probe_at_candA_11_12_1 (x : Int) (h_x_bound : 0 ≤ x ∧ x < 18446744073709551616) :
    /- @rust:/tmp/ch5probe/candA.rs:11:12 -/ Int.toNat x * Int.toNat x = sq (Int.toNat x) := by
  intros; simp only [sq] 
theorem _tactus_ensures_probe_2 (x : Int) (h_x_bound : 0 ≤ x ∧ x < 18446744073709551616) :
    Int.toNat x * Int.toNat x = sq (Int.toNat x) → Int.toNat x * Int.toNat x = sq (Int.toNat x) → True := by
  tactus_auto
end candA
