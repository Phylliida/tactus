...
fn resolve_lean_path() -> String {
    ...
    // (CRATEDEFS.md step 0) from the fixed user-level cache dir. No
    // version compatibility: artifacts generated against an older
    // prelude fail and are regenerated by re-running tactus.
    match prelude_cache_dir() {
...
/// The fixed prebuilt-prelude cache dir, if it exists. Mirrors
/// `lean_verify::prelude::prelude_cache_dir`'s root fallbacks.
fn prelude_cache_dir() -> Option<String> {
    let root = if let Ok(d) = std::env::var("TACTUS_PRELUDE_CACHE") {
        ...
        std::path::PathBuf::from(std::env::var("HOME").ok()?).join(".cache").join("tactus")
    };
    let dir = root.join("prelude");
    dir.join("TactusPrelude.olean").exists()
