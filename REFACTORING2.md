...
| 1.4 god-function splits | **krate_preamble: split landed via 2.B step 1a** (`spec_world_cmds` extracted, ~470 lines; the survey's predicted deletion is deferred until/unless defs mode becomes the default and standalone retires — wait on real-crate experience). build_wp / push_post_call_frames splits remain open | `d8f0a7a` |
...
| EmitCtx follow-ups | open — `RenderCtx::empty()` upgrade sites (behavior changes, one at a time with e2e); thread-local fold-in **now UNBLOCKED** (2.A complete): the two remaining ambient tables (`to_lean_type.rs:294` renames, `to_lean_sst_expr.rs:256` field bounds) can fold into EmitCtx/RenderCtx — finishes 1.2's vision, kills installation-order-dependent state | |
