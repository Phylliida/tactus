...
| 2.B shared crate module | **design pass DONE** (same day) → `CRATEDEFS.md`, with measurements: inline prelude costs ~1.3s/check (~2.6s → ~1.3s with prebuilt .olean; one-time 2.7s build). Three gated steps: prelude-.olean (do first, ~2× suite CPU win) → CrateDefs for ≥2-fn crates (the ~800-line deletion) → worker pool (measure first) | |
| 2.C persistent Lean worker | folded into `CRATEDEFS.md` step 2 — the warm-worker infra already EXISTS (`tactus-lsp`, ~2ms warm queries); batch variant = same pattern returning diagnostics; gate on post-step-0/1 measurement | |
