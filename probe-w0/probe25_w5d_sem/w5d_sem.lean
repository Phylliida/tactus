...
-- ══════════════════════════════════════════════════════════════════════
-- W5d PROBE (board bootstrap-52) — SOUNDNESS of the reference WP for `&mut` /
-- PROPHECY (the ∀-final-value model, master plan O5), proven over the REAL
-- emitted `lib.wp_stm` / `lib.frame_after` / `lib.frame_append` / `lib.close_e`
-- (tactus-core/out/lib), NO tactus-core rebuild.
--
-- CARRIES OVER the full W5c core (probe24): the frame-carrying `execSafeF`, the
-- iff soundness theorem `wp_stm_sound : holdsAll (wp_stm f s) st ↔ execSafeF f s
-- st` TOTAL over the whole StmData vocabulary (Skip/Assume/Assign/Assert/Call/
-- Ret/DeadEnd/If/Seq/Loop) over an ARBITRARY frame telescope. W5d does NOT add a
-- new arm: prophecy is modeled ENTIRELY at the frame/statement level
-- (DESIGN-W2-refwp §2.6 — `&mut` post-state flows through the same `post:
-- FrameList` / statement machinery as `Call`, no new StmData constructor).
--
-- HOW VERUS ACTUALLY ENCODES PROPHECY (verified from the verus source, NOT
-- reasoned from first principles):
--   • `&mut x` introduces a FRESH prophesied FINAL value `x_fut`, ∀-quantified
--     (the "standard trick", master plan O5). In the frame this is an `FBind
--     x_fut ty` (borrow creation / Call post-frame).
...
--     (verus/source/vir/src/ast.rs:1087 "assume(has_resolved(place))";
--     resolution_inference.rs:77 "insert Assume(HasResolved(p))"). So `resolve`
--     is a `StmData.Assume`, whose `frame_after` threads `FHyp(x==x_fut)` into
--     the CONTINUATION — the pin lands at the resolution point, downstream of
--     the mutations, NOT before the body.
--
-- WHY THIS IS ALREADY SOUND (and NON-VACUOUS): `closeSem`'s `FBind` arm IS the
-- ∀-final quantification (∀ n, closeSem tail (upd st x_fut n) body); `frame_after
-- (Assume resolve) = frame_append f (FHyp resolve)` places the resolve pin as a
-- continuation hypothesis (the emitted reference, u_fa_assume). So `wp_stm` for
-- the canonical caller shape `resolve; assert P(*x)` reduces EXACTLY to
--   ∀ x_fut, resolve(x_fut) → P(x_fut)         (prophecy_sound)
-- — the ∀-final-value model + resolve pin, made explicit and machine-checked.
--
-- TEMPORAL-PLACEMENT WITNESS (Danielle's local model flagged the worry that an
-- FHyp shifts the pin to the WRONG end of the borrow): we prove `resolve; assert`
-- gives `∀ x_fut, resolve → P` while the SWAPPED `assert; resolve` gives the
-- UNGATED `∀ x_fut, P` — the reference's `frame_after` places the pin
...
--
-- Design + model: DESIGN-W5-soundness.md §4 (W5d row) + §1 (O5) + board
-- bootstrap-52. Extends probe24 (W5c).
-- ══════════════════════════════════════════════════════════════════════
...
-- emitted reference: frame_append (tail-append) + frame_after Assume/Assert
-- (the resolve-as-Assume threading — u_fa_assume is the load-bearing fact that
-- `resolve` pins x==x_fut into the CONTINUATION, not the pre-body frame).
...
-- ══════════════════════════════════════════════════════════════════════
-- W5d: `&mut` / PROPHECY — the ∀-final-value model, instantiated + witnessed.
-- No new StmData arm; prophecy IS the frame telescope + `Assume`-resolve.
-- ══════════════════════════════════════════════════════════════════════

-- The borrow frame after `&mut x` introduces the prophesied FINAL value as a
-- FRESH ∀-binder `FBind x_fut ty` (nothing about it is assumed yet — the pin
-- comes later, at resolve).
def prophecyFrame (xfut ty : Int) : lib.FrameList :=
  lib.FrameList.FBind xfut ty (Tactus.Box.mk lib.FrameList.FNil)

-- The canonical caller-side prophecy program: `resolve; assert P(*x)`, where
-- `*x` (post-borrow) reads the prophesied `x_fut`. `resolve` is a
-- `StmData.Assume` (Verus's `Assume(has_resolved)`); the assert's obligation
-- `obl` is the deep property over the final value (opaque `he ∘ render_exp`).
def prophecyProg (resolve : Int) (obl : lib.RawExp) (h : Int) : lib.StmData :=
  lib.StmData.Seq
    (Tactus.Box.mk (lib.StmData.Assume resolve))
    (Tactus.Box.mk (lib.StmData.Assert obl h))

-- The SWAPPED program `assert P(*x); resolve` — resolve AFTER the assert. Used
-- to witness that placement is temporally meaningful (the pin does NOT reach an
-- upstream obligation).
def prophecyProgSwapped (resolve : Int) (obl : lib.RawExp) (h : Int) : lib.StmData :=
  lib.StmData.Seq
    (Tactus.Box.mk (lib.StmData.Assert obl h))
    (Tactus.Box.mk (lib.StmData.Assume resolve))

-- ── W5d MAIN (prophecy_sound). The reference WP for `resolve; assert P(*x)`
--    over the borrow frame reduces EXACTLY to the ∀-final-value reading:
--      ∀ x_fut, resolve(x_fut) → P(x_fut).
--    i.e. the obligation must hold for EVERY prophesied final value, UNDER the
--    resolve pin. Proven by instantiating the W5c iff `wp_stm_sound` and
--    unfolding execSafeF's Seq/Assume/Assert arms + the emitted frame threading.
theorem prophecy_sound (hp : Int → St → Prop) (he : lib.ExprData → St → Prop)
    (lv : Int → St → Int) (xfut ty resolve h : Int) (obl : lib.RawExp) (st : St) :
    holdsAll hp he lv (lib.wp_stm (prophecyFrame xfut ty) (prophecyProg resolve obl h)) st
      ↔ (∀ n : Int, hp resolve (upd st xfut n) → he (lib.render_exp obl) (upd st xfut n)) := by
  rw [wp_stm_sound hp he lv (prophecyProg resolve obl h) (prophecyFrame xfut ty) st]
  simp only [prophecyProg, prophecyFrame, u_exec_seq, u_exec_assume, true_and,
             u_fa_assume, u_fapp_fbind, u_fapp_fnil, u_exec_assert,
             u_cs_FBind, u_cs_FHyp, u_cs_FNil]

-- ── W5d SWAPPED (prophecy_swapped_sound). `assert P(*x); resolve` reduces to
--    the UNGATED `∀ x_fut, P(x_fut)` — resolve comes AFTER, so the pin never
--    reaches the assert obligation. (The trailing `Assume` emits no goal; its
--    only WP effect is on a — here absent — continuation.)
theorem prophecy_swapped_sound (hp : Int → St → Prop) (he : lib.ExprData → St → Prop)
    (lv : Int → St → Int) (xfut ty resolve h : Int) (obl : lib.RawExp) (st : St) :
    holdsAll hp he lv (lib.wp_stm (prophecyFrame xfut ty) (prophecyProgSwapped resolve obl h)) st
      ↔ (∀ n : Int, he (lib.render_exp obl) (upd st xfut n)) := by
  rw [wp_stm_sound hp he lv (prophecyProgSwapped resolve obl h) (prophecyFrame xfut ty) st]
  simp only [prophecyProgSwapped, prophecyFrame, u_exec_seq, u_exec_assert,
             u_exec_assume, and_true, u_cs_FBind, u_cs_FNil]

-- ── W5d NON-VACUITY (∀-final). From the emitted goals holding, the obligation
--    holds for EVERY prophesied final value (opaque `he` — it can only come from
--    the emitted goal). If the model dropped the ∀-final binder (checked one
--    value), this could not be derived.
example (hp : Int → St → Prop) (he : lib.ExprData → St → Prop) (lv : Int → St → Int)
    (xfut ty resolve h : Int) (obl : lib.RawExp) (st : St)
    (hg : holdsAll hp he lv (lib.wp_stm (prophecyFrame xfut ty) (prophecyProg resolve obl h)) st) :
    ∀ n : Int, hp resolve (upd st xfut n) → he (lib.render_exp obl) (upd st xfut n) :=
  (prophecy_sound hp he lv xfut ty resolve h obl st).mp hg

-- ── W5d TEMPORAL-PLACEMENT (the local model's worry, discharged). Placement of
--    the resolve pin is NOT vacuous: `resolve; assert` gates the obligation with
--    `hp resolve`, whereas the swapped `assert; resolve` demands the UNGATED
--    obligation. Concretely: a state that discharges the correctly-placed program
--    (needs resolve to bite) need NOT discharge the swapped one — the pin lands
--    downstream of resolve, exactly as `frame_after (Assume _)` dictates. We
--    witness the gate is REAL by exhibiting the two distinct reduced forms.
example (hp : Int → St → Prop) (he : lib.ExprData → St → Prop) (lv : Int → St → Int)
    (xfut ty resolve h : Int) (obl : lib.RawExp) (st : St) :
    (holdsAll hp he lv (lib.wp_stm (prophecyFrame xfut ty) (prophecyProg resolve obl h)) st
       ↔ (∀ n : Int, hp resolve (upd st xfut n) → he (lib.render_exp obl) (upd st xfut n)))
    ∧ (holdsAll hp he lv (lib.wp_stm (prophecyFrame xfut ty) (prophecyProgSwapped resolve obl h)) st
       ↔ (∀ n : Int, he (lib.render_exp obl) (upd st xfut n))) :=
  ⟨prophecy_sound hp he lv xfut ty resolve h obl st,
   prophecy_swapped_sound hp he lv xfut ty resolve h obl st⟩
...
#print axioms W5d.wp_stm_sound          -- expect [propext, Quot.sound]
#print axioms W5d.ref_wp_sound          -- expect [propext, Quot.sound]
#print axioms W5d.prophecy_sound        -- expect [propext, Quot.sound]
#print axioms W5d.prophecy_swapped_sound -- expect [propext, Quot.sound]
