...
-- W5f v2: `eval`/`evalList` are a MUTUAL STRUCTURAL pair (AppN folds its arg list).
-- Comparison / logical ops now carry a DECIDABLE Bool-as-Int value (`if P then 1
-- else 0`, P decidable on Int ⇒ no `Classical`) so an Ite whose condition is a
-- comparison denotes faithfully: `eval c ≠ 0 ↔ edenote c`. Match stays a scoped
-- sentinel (flat-Int scrutinee decode is a datatype-value-decode follow-on;
-- match-BODIED fns are reached through the pinned `fn`/`fnN` oracle instead).
mutual
def eval (E : SymEnv) (e : lib.ExprData) (st : St) : Int :=
  ...
  | lib.ExprData.Match _ _ => 0                                        -- scoped: flat-Int decode follow-on
...
def edenote (E : SymEnv) (e : lib.ExprData) (st : St) : Prop :=
  ...
  | lib.ExprData.Match _ _ => True                                    -- scoped: flat-Int decode follow-on
termination_by structural e
...
-- ══════════════════════════════════════════════════════════════════════
-- W5f v2 — the FOUR body-fragment adequacy facts, each over the REAL
-- `lib.render_exp` (App/AppN/Forall/Ite arms), pinning the meaning of the
-- widened `eval`/`edenote`. Match is scoped (see the SymEnv comment).
-- ══════════════════════════════════════════════════════════════════════
...
-- axioms. `render_exp` reduces (structural), `edenote`/`eval`/`evalList` reduce
-- (mutual structural). The v2 body-fragment facts (App/AppN/Forall/Ite) add NO
-- new axioms — the Ite decidability is `Int.decEq` (constructive), the quantifier
-- is a genuine `∀`, the fn grounding is a plain oracle application.
