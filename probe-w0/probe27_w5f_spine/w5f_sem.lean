...
structure SymEnv where
  ...
  fn   : Int → Int → Int       -- App fn id → unary spec-fn interpretation
...
def eval (E : SymEnv) (e : lib.ExprData) (st : St) : Int :=
  ...
  | lib.ExprData.BinOp op l r =>
      (match E.opk op with
       ...
       | _ => 0)                                                       -- comparison/logical op in value position: sort-error sentinel
  ...
  | lib.ExprData.Not _ => 0                                            -- Prop in value position: sentinel
  | lib.ExprData.Ite _ _ _ => 0                                        -- deeper fragment (W5f v2): sentinel
  | lib.ExprData.Match _ _ => 0
  | lib.ExprData.AppN _ _ => 0
  | lib.ExprData.Forall _ _ _ => 0
...
def edenote (E : SymEnv) (e : lib.ExprData) (st : St) : Prop :=
  ...
  | lib.ExprData.App _ _ => eval E e st ≠ 0
  | lib.ExprData.FieldProj _ _ => eval E e st ≠ 0
  | lib.ExprData.Let n v b => edenote E b.deref (upd st n (eval E v.deref st))
  | lib.ExprData.Ite _ _ _ => True                                    -- deeper fragment (W5f v2)
  | lib.ExprData.Match _ _ => True
  | lib.ExprData.AppN _ _ => True
  | lib.ExprData.Forall _ _ _ => True
  | lib.ExprData.Exists _ _ _ => True
termination_by structural e
...
-- axioms. `render_exp` reduces (structural), `edenote`/`eval` reduce (structural).
#print axioms W5f.adequacy_leaf_cmp        -- expect [propext, Quot.sound]
#print axioms W5f.adequacy_leaf_overflow   -- expect [propext, Quot.sound]
#print axioms W5f.toProp_all_embed         -- expect [propext, Quot.sound]
#print axioms W5f.soundness_concrete       -- expect [propext, Quot.sound]
#print axioms W5f.wp_stm_sound             -- expect [propext, Quot.sound]
