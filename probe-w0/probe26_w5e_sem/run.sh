#!/usr/bin/env bash
# W5e soundness probe runner (board bootstrap-53). Elaborates the standalone
...
#
# Usage: probe-w0/probe26_w5e_sem/run.sh          (LEAN=<lean> to override)
set -uo pipefail
...
SRC="$HERE/w5e_sem.lean"

export LEAN_PATH="$CORE_OUT:$PRELUDE"

echo "== W5e reference-WP soundness probe (closures: DeadEnd isolation + Assume forwarding) =="
...
if [ $rc -eq 0 ]; then
  echo "PASS ✓ — reference WP is sound for closures (DeadEnd isolation + Assume forwarding) (rc=0)"
else
  echo "FAIL ✗ — rc=$rc"
fi
exit $rc
