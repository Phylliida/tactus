...
fn generate_lean(
    ...
    let refs = dep_order::collect_references(&all_fns, &[proof_fn]);

    // Method lookup for trait class emission
    ...
    let groups = dep_order::order_spec_fns(&all_fns, &[proof_fn]);
    for group in &groups {
        match group {
            FnGroup::Single(f) => {
    ...
    // 4. Proof fn theorem
    let mut source_map = to_lean_fn::LeanSourceMap { proof_fns: Vec::new() };
    to_lean_fn::write_proof_fn(&mut out, proof_fn, tactic_body);
    let fn_name = short_name(&proof_fn.name.path).to_string();
    let total_lines = out.chars().filter(|&c| c == '\n').count();
    let tactic_line_count = tactic_body.lines().count().max(1);
    source_map.proof_fns.push(to_lean_fn::ProofFnMapping {
        fn_name,
        tactic_start_line: total_lines + 1 - tactic_line_count,
        tactic_line_count,
    });
    out.push('\n');
