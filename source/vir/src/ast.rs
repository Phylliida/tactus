...
pub struct FunctionAttrsX {
    ...
    /// (file_path, start_byte, end_byte) — the byte range of `{ ... }` in the source file.
    /// The file path is resolved at VIR construction time (when SourceMap is available).
    pub tactic_span: Option<(String, usize, usize)>,
