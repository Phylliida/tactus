            ...
            if is_invariant_block(bctx, expr)? {
                    ...
                    && crate::fn_call_to_vir::enclosing_fn_is_tactus_auto(bctx)
                {
                    ...
                    if tactic_span.is_some() {
                        ...
                        let empty_block = bctx.spanned_typed_new(
                            expr.span,
                            &vir::ast_util::unit_typ(),
                            ExprX::Block(Arc::new(vec![]), None),
                        );
                        let assert_by = bctx.spanned_typed_new(
                            expr.span,
                            &vir::ast_util::unit_typ(),
                            ExprX::AssertBy {
                                vars: Arc::new(vec![]),
                        ...
                        // treats this as a proof-mode construct
                        // (AssertBy is invalid in direct exec mode).
                        // `ast_simplify` unwraps Ghost, leaving the
                        // AssertBy for `ast_to_sst`'s Tactus path
                        // to route to `StmX::AssertQuery`.
                        return mk_expr(ExprX::Ghost {
                            alloc_wrapper: false,
                            tracked: false,
                            expr: assert_by,
                        });
